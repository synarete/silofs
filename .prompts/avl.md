...
contract. A comparison that looks backwards under standard semantics is
likely correct here.
...
- Review `avl_delete_fixup`. Pay special attention to the case where the
  sibling node has a balance of 0, which is a classic AVL pitfall.
- Analyze `avl_delete` node swapping logic. Ensure that when a node with
  ...
  replaces the target node without breaking `parent` pointers.
...
- Verify that leftmost/rightmost are updated **before** `avl_insert_fixup`
  completes, as rotations may change node relationships.
...
### 3.5 Memory Safety and API
- Audit `avl_node_verify`. Check if the magic value and balance factor
  assertions are sufficient to detect corruption.
- Review `avl_unlinkall`. Ensure it transforms the tree into a linear list
  using the `right` pointer without losing nodes.
- Check `avl_node_unconst`. Verify the union-based cast does not violate
...
- **Edge Cases:** Failures during root deletion or empty tree operations.
- **Comparator Issues:** Mismatches between implementation and the contract.
