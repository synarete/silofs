...
cmd_fsids_save_at(const struct silofs_ugids *fsids, int dfd, const char *name)
{
	...
	cmd_json_save_at(jfsids, dfd, name);
                    ...
                    const struct silofs_boot_args *boot_args)
{
	...
	cmd_fsids_save_at(fsids, dfd, cmd_jfsids_filename);
...
cmd_fsids_load_at(struct silofs_ugids *fsids, int dfd, const char *name)
{
	...
	jfsids = cmd_json_load_at(dfd, name);
                    ...
                    const struct silofs_boot_args *boot_args)
{
	...
	cmd_open_jconfdir(boot_args, &dfd);
	cmd_fsids_load_at(fsids, dfd, cmd_jfsids_filename);
	cmd_close_jconfdir(boot_args, dfd);
}
