...
static void print_spacestats(const struct silofs_spacestats *spst)
{
	...
	prefix = "lexts";
	print_count(prefix, "ndata1k", spst->lexts.ndata1k);
	print_count(prefix, "ndata4k", spst->lexts.ndata4k);
	print_count(prefix, "ndatabk", spst->lexts.ndatabk);
	print_count(prefix, "nsuper", spst->lexts.nsuper);
	print_count(prefix, "nspnode", spst->lexts.nspnode);
	print_count(prefix, "nspleaf", spst->lexts.nspleaf);
	print_count(prefix, "ninode", spst->lexts.ninode);
	print_count(prefix, "nxanode", spst->lexts.nxanode);
	print_count(prefix, "ndtnode", spst->lexts.ndtnode);
	print_count(prefix, "nftnode", spst->lexts.nftnode);
	print_count(prefix, "nsymval", spst->lexts.nsymval);
	prefix = "bks";
