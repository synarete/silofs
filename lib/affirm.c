...
static void affirm_ondisk_headers(void)
{
	...
	REQUIRE_OFFSET32(struct silofs_header, h_type, 8);
...
static void affirm_ondisk_spmaps(void)
{
	...
	REQUIRE_OFFSET64(struct silofs_spmap_node, sn_self, 224 + 32);
	...
	REQUIRE_OFFSET64(struct silofs_spmap_leaf, sl_self, 224 + 32);
...
static void affirm_ondisk_bdesc(void)
{
	...
	REQUIRE_OFFSET64(struct silofs_blob_desc, bd_refblob, 96);
	REQUIRE_OFFSET64(struct silofs_blob_desc, bd_blobsize, 128);
	REQUIRE_OFFSET64(struct silofs_blob_desc, bd_objsize, 136);
