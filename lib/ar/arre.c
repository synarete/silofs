...
static bool adi_ismrec(const struct silofs_ar_desc_info *adi)
{
...
static void arix_link_desc(struct silofs_ar_index *arix,
                           struct silofs_ar_desc_info *adi, bool front)
{
	if (front) {
		silofs_listq_push_front(&arix->descq, &adi->lh);
	} else {
		silofs_listq_push_back(&arix->descq, &adi->lh);
	}
}

static void
arix_unlink_desc(struct silofs_ar_index *arix, struct silofs_ar_desc_info *adi)
{
	silofs_listq_remove(&arix->descq, &adi->lh);
}

static struct silofs_ar_desc_info *
arix_add_desc(struct silofs_ar_index *arix, const struct silofs_laddr *laddr,
...
static void
arix_rm_desc(struct silofs_ar_index *arix, struct silofs_ar_desc_info *adi)
{
	arix_unlink_desc(arix, adi);
...
arix_next_desc(const struct silofs_ar_index *arix,
...
static void arix_clear_descq(struct silofs_ar_index *arix)
{
	struct silofs_ar_desc_info *adi;
...
static size_t arix_ndescs_inq(const struct silofs_ar_index *arix)
{
	return arix->descq.sz;
}

static size_t arix_size_of(size_t ndesc)
{
...
static int arix_init(struct silofs_ar_index *arix, struct silofs_alloc *alloc)
{
	silofs_listq_init(&arix->descq);
	arix->alloc = alloc;
	return silofs_mdigest_init(&arix->mdigest);
}

static void arix_fini(struct silofs_ar_index *arix)
{
...
{
	return arix_size_of(arix_ndescs_inq(arix));
}
...
static void arix_encode_meta(const struct silofs_ar_index *arix,
                             struct silofs_ar_index_view *aiview)
{
	silofs_unused(arix);
...
static int arix_decode_meta(struct silofs_ar_index *arix,
                            struct silofs_ar_index_view *aiview)
{
	int err;
...
static void arix_calc_caddr_of(const struct silofs_ar_index *arix,
                               const struct silofs_ar_index_view *aiview,
                               struct silofs_caddr *out_caddr)
{
	aiview_calc_caddr(aiview, &arix->mdigest, out_caddr);
}
                       ...
                       struct silofs_caddr *out_caddr)
{
	...
	err = arix_encode_descs(arix, &aiview);
	if (err) {
		return err;
	}
	arix_encode_meta(arix, &aiview);
	arix_calc_caddr_of(arix, &aiview, out_caddr);
	return 0;
}

static int arix_check_caddr(const struct silofs_ar_index *arix,
                            const struct silofs_caddr *caddr,
                            const struct silofs_ar_index_view *aiview)
{
	struct silofs_caddr caddr_calc;

	aiview_calc_caddr(aiview, &arix->mdigest, &caddr_calc);
            ...
            const struct silofs_rovec *rov)
{
	...
	err = arix_check_caddr(arix, caddr, &aiview);
	if (err) {
		return err;
	}
	err = arix_decode_meta(arix, &aiview);
	if (err) {
		return err;
	}
	err = arix_decode_descs(arix, &aiview);
	if (err) {
		return err;
	}
	return 0;
...
static int arc_init(struct silofs_ar_ctx *ar_ctx, struct silofs_task_ctx *task)
{
	...
	ar_ctx->task = task;
	ar_ctx->env = env;
	return arix_init(&ar_ctx->arix, arc_alloc(ar_ctx));
}
...
static int arc_load_fs_mrec(const struct silofs_ar_ctx *ar_ctx,
                            const struct silofs_caddr *caddr,
                            struct silofs_mrec1k *out_mrec1k)
{
	struct silofs_mrec mrec = { .flags = SILOFS_MRECF_NONE };
	int err;

	err = silofs_load_mrec(ar_ctx->env, caddr, &mrec);
	if (err) {
		log_err("failed to load mrec: err=%d", err);
		return err;
	}
	err = silofs_encode_mrec(ar_ctx->env, &mrec, out_mrec1k);
	if (err) {
...
static int arc_restore_fs_mrec(const struct silofs_ar_ctx *ar_ctx,
                               const struct silofs_caddr *caddr,
                               struct silofs_mrec1k *mrec1k)
{
	...
	err = silofs_decode_mrec(ar_ctx->env, mrec1k, &mrec);
	if (err) {
		return err;
	}
	/* TODO: check proper caddr before save */
	err = silofs_save_mrec(ar_ctx->env, &mrec, &caddr2);
	if (err) {
		return err;
	}
...
static int arc_fs_mrec_caddr(const struct silofs_ar_ctx *ar_ctx,
                             struct silofs_caddr *out_caddr)
{
	return silofs_env_mrec_addr(ar_ctx->env, out_caddr);
}

static int arc_archive_mrec(const struct silofs_ar_ctx *ar_ctx,
                            struct silofs_ar_desc_info *adi)
{
	...
	err = arc_fs_mrec_caddr(ar_ctx, &caddr);
	if (err) {
		return err;
	}
	err = arc_load_fs_mrec(ar_ctx, &caddr, &mrec1k);
	if (err) {
		return err;
	}
	adi_update_caddr(adi, &caddr);

	err = arc_send_pack(ar_ctx, &adi->ard.caddr, &mrec1k, sizeof(mrec1k));
	if (err) {
		return err;
	}
	return 0;
...
static int arc_archive_mrec2(const struct silofs_ar_ctx *ar_ctx,
                             struct silofs_caddr *out_caddr)
{
	struct silofs_mrec1k mrec1k = { .mrec_magic = 0xff };
	...
	err = silofs_encode_mrec(ar_ctx->env, mrec, &mrec1k);
	if (err) {
		return err;
	}
	err = arc_send_pack(ar_ctx, out_caddr, &mrec1k, sizeof(mrec1k));
	if (err) {
		return err;
	}
	return 0;
...
static int arc_restore_mrec(const struct silofs_ar_ctx *ar_ctx,
                            const struct silofs_ar_desc_info *adi)
{
	struct silofs_mrec1k mrec1k = { .mrec_magic = 0xff };
	...
	err = arc_recv_pack(ar_ctx, caddr, &mrec1k, sizeof(mrec1k));
	if (err) {
		return err;
	}
	err = arc_restore_fs_mrec(ar_ctx, caddr, &mrec1k);
	if (err) {
		return err;
	}
	return 0;
                               ...
                               struct silofs_ar_desc_info *adi)
{
	...
	if (adi_ismrec(adi)) {
		err = arc_archive_mrec(ar_ctx, adi);
	} else {
...
static int
arc_encode_save_arix(struct silofs_ar_ctx *ar_ctx, struct silofs_bytebuf *bb,
                     struct silofs_caddr *out_caddr)
{
	...
	err = arix_encode(&ar_ctx->arix, &rwv, out_caddr);
	if (err) {
		return err;
	}
...
static int arc_acquire_enc_buf(const struct silofs_ar_ctx *ar_ctx,
                               struct silofs_bytebuf *out_bbuf)
{
	const size_t bsz = arix_encsize(&ar_ctx->arix);
	int err;
...
arc_archive_post(struct silofs_ar_ctx *ar_ctx, struct silofs_caddr *out_caddr)
{
	...
	err = arc_archive_mrec2(ar_ctx, out_caddr);
	if (err) {
		return err;
	}
	return 0;
...
static int arc_load_decode_arix(struct silofs_ar_ctx *ar_ctx,
                                const struct silofs_caddr *caddr,
                                struct silofs_bytebuf *bb)
{
	...
	err = arix_decode(&ar_ctx->arix, caddr, &rov);
	if (err) {
		return err;
	}
	return 0;
...
static int arc_stat_arix(const struct silofs_ar_ctx *ar_ctx,
                         const struct silofs_caddr *caddr, size_t *out_sz)
{
	int err;
	...
	if (err) {
		log_warn("illegal archive arix: size=%zu", *out_sz);
		return -SILOFS_EINVAL;
	}
	return 0;
}
                               ...
                               const struct silofs_ar_desc_info *adi)
{
	...
	if (adi_ismrec(adi)) {
		err = arc_restore_mrec(ar_ctx, adi);
	} else {
...
static int arc_restore_post(struct silofs_ar_ctx *ar_ctx)
{
	...
	silofs_env_set_mrec_addr(ar_ctx->env, &caddr);
	return 0;
}
