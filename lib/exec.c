...
{
	const struct silofs_cred *host_cred = &task->t_auth.creds.host_cred;
	struct silofs_cred       *fs_cred   = &task->t_auth.creds.fs_cred;
	int                       ret       = 0;
                      ...
                      struct silofs_stat *out_stat)
{
	...
	err = op_stage_mut_inode(task, parent, NULL, &dir_ii);
	ok_or_goto_out(err);
                      ...
                      struct silofs_stat         *out_stat)
{
	...
	err = op_stage_mut_inode(task, ino, NULL, &ii);
	ok_or_goto_out(err);
                       ...
                       const char *newname, int flags)
{
	...
	err = op_stage_mut_inode(task, parent_ino, NULL, &curd_ii);
	ok_or_goto_out(err);
                     ...
                     int iflags_dont)
{
	...
	err = op_stage_mut_inode(task, ino, NULL, &dir_ii);
	ok_or_goto_out(err);
                        ...
                        struct silofs_paddr    *out_ar_paddr)
{
	...
	err = silofs_do_archive_fs(task, out_ar_paddr);
	ok_or_goto_out(err);
out:
	return op_finish(task, err);
}
