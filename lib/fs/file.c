...
static off_t silofs_off_max3(off_t off1, off_t off2, off_t off3)
{
...
static off_t off_in_data(off_t off, enum silofs_mtype mtype)
{
...
static size_t len_to_next(off_t off, enum silofs_mtype mtype)
{
	const ssize_t len = silofs_mtype_ssize(mtype);
...
static size_t len_of_data(off_t off, off_t end, enum silofs_mtype mtype)
{
	const ssize_t len = silofs_mtype_ssize(mtype);
	const off_t next  = likely(len > 0) ? silofs_off_next(off, len) : off;
...
static bool off_is_partial(off_t off, off_t end, enum silofs_mtype mtype)
{
	const ssize_t len = silofs_mtype_ssize(mtype);
...
{
	return off_is_partial(off, end, SILOFS_MTYPE_DATA1K);
}

static bool off_is_partial_head2(off_t off, off_t end)
{
	return off_is_partial(off, end, SILOFS_MTYPE_DATA4K);
}
...
static enum silofs_mtype fli_mtype(const struct silofs_ftleaf_info *fli)
{
	return silofs_vni_mtype(&fli->ftl_vni);
}

static size_t fli_data_len(const struct silofs_ftleaf_info *fli)
{
	return silofs_mtype_size(fli_mtype(fli));
}
...
static void *fli_data_at(const struct silofs_ftleaf_info *fli, off_t pos)
{
	...
	const enum silofs_mtype mtype = fli_mtype(fli);

	if (mtype == SILOFS_MTYPE_DATA1K) {
...
{
	return off_in_data(off, fli_mtype(fli));
}
...
static enum silofs_mtype ftn_child_mtype(const struct silofs_ftree_node *ftn)
{
	return (enum silofs_mtype)(ftn->fn_child_mtype);
}

static void
ftn_set_child_mtype(struct silofs_ftree_node *ftn, enum silofs_mtype mtype)
{
	ftn->fn_child_mtype = (uint8_t)(mtype);
}

static void
ftn_child_mtype_by_height(const struct silofs_ftree_node *ftn, size_t height,
...
{
	off_t span              = 0;
	const off_t bk_size     = SILOFS_FILE_TREE_LEAF_SIZE;
...
static void ftn_init(struct silofs_ftree_node *ftn, ino_t ino, off_t beg,
                     off_t end, size_t height, enum silofs_mtype child_mtype)
{
	...
	ftn_set_child_mtype(ftn, child_mtype);
...
ftn_init_by(struct silofs_ftree_node *ftn, ino_t ino, off_t off, size_t height)
{
	...
	ftn_child_mtype_by_height(ftn, height, &child_mtype);
	ftn_calc_range(ftn, off, height, &beg, &end);
	ftn_init(ftn, ino, beg, end, height, child_mtype);
}
                                      ...
                                      size_t slot, struct silofs_vaddr *vaddr)
{
	...
	silofs_vaddr_setup(vaddr, ftn_child_mtype(ftn), off);
}
...
static void filc_iovec_by_nilbk(const struct silofs_file_ctx *f_ctx,
                                const enum silofs_mtype mtype,
                                struct silofs_iovec *out_iov)
{
	silofs_iovec_reset(out_iov);
	out_iov->iov.iov_base = filc_nil_block(f_ctx);
	out_iov->iov.iov_len  = len_of_data(f_ctx->off, f_ctx->end, mtype);
	out_iov->iov_off      = 0;
}
                                  ...
                                  const struct silofs_vaddr *vaddr)
{
	...
	return silofs_resolve_llink_of(f_ctx->ectx, vaddr, stg_mode, &llink);
}
...
static bool filc_has_tree_root(const struct silofs_file_ctx *f_ctx)
{
	...
	return (vaddr.mtype == SILOFS_MTYPE_FTNODE);
}
...
static size_t filc_distance_to_next(const struct silofs_file_ctx *f_ctx)
{
	...
	filc_curr_data_mtype(f_ctx, &mtype);
	return len_to_next(f_ctx->off, mtype);
}
...
static void filc_update_post_io(const struct silofs_file_ctx *f_ctx)
{
	...
	silofs_update_iattrs_of(f_ctx->ectx, ii, &iattr);
}
                                    ...
                                    struct silofs_fileaf_ref *flref)
{
	...
	return silofs_vaddr_isnull(&flref->vaddr) ?
	               0 :
	               silofs_test_unwritten_at(f_ctx->ectx, &flref->vaddr,
	                                        &flref->unwritten);
}
                             ...
                             struct silofs_ftleaf_info **out_fli)
{
	...
	err = silofs_stage_vnode(f_ctx->ectx, f_ctx->ii, vaddr,
	                         f_ctx->stg_mode, &vni);
	if (err) {
		return err;
	}
                               ...
                               struct silofs_iovec *out_iov)
{
	...
	if (fli != nullptr) {
		...
		filc_curr_data_mtype(f_ctx, &mtype);
		filc_iovec_by_nilbk(f_ctx, mtype, out_iov);
	}
}
...
int silofs_do_read_iter(struct silofs_exec_ctx *ectx,
...
int silofs_do_read(struct silofs_exec_ctx *ectx, struct silofs_inode_info *ii,
...
static int filc_clear_unwritten_at(const struct silofs_file_ctx *f_ctx,
                                   const struct silofs_vaddr *vaddr)
{
	return silofs_clear_unwritten_at(f_ctx->ectx, vaddr);
}
...
static int
filc_claim_vspace(const struct silofs_file_ctx *f_ctx, enum silofs_mtype mtype,
                  struct silofs_vaddr *out_vaddr)
{
	return silofs_claim_vspace(f_ctx->ectx, mtype, out_vaddr);
}

static int
filc_claim_data_space(const struct silofs_file_ctx *f_ctx,
                      enum silofs_mtype mtype, struct silofs_vaddr *out_vaddr)
{
	int err;

	err = filc_claim_vspace(f_ctx, mtype, out_vaddr);
	if (err) {
		return err;
	}
...
static int filc_share_data_space(const struct silofs_file_ctx *f_ctx,
                                 const struct silofs_vaddr *vaddr)
{
	return silofs_addref_vspace(f_ctx->ectx, vaddr);
}

static int filc_reclaim_data_space(const struct silofs_file_ctx *f_ctx,
                                   const struct silofs_vaddr *vaddr)
{
	return silofs_reclaim_vspace(f_ctx->ectx, vaddr);
}
                               ...
                               const struct silofs_vaddr *vaddr)
{
	...
	err = silofs_test_last_allocated(f_ctx->ectx, vaddr, &last);
	if (err) {
		return err;
	}
	...
	err = silofs_remove_vnode_at(f_ctx->ectx, vaddr);
	if (err) {
		return err;
	}
	return 0;
                             ...
                             struct silofs_ftnode_info **out_fni)
{
	...
	err = silofs_spawn_vnode(f_ctx->ectx, f_ctx->ii, SILOFS_MTYPE_FTNODE,
	                         &vni);
	if (err) {
		return err;
	}
...
static int filc_remove_finode(const struct silofs_file_ctx *f_ctx,
                              struct silofs_ftnode_info *fni)
{
	return silofs_remove_vnode(f_ctx->ectx, &fni->ftn_vni);
}
...
{
	silofs_update_iblocks_of(f_ctx->ectx, f_ctx->ii, vaddr->mtype, dif);
}
                      ...
                      enum silofs_mtype mtype, struct silofs_vaddr *out_vaddr)
{
	...
	err = filc_claim_data_space(f_ctx, mtype, out_vaddr);
	if (err) {
		return err;
	}
                                        ...
                                        struct silofs_fileaf_ref *out_flref)
{
	...
	err = filc_create_data_leaf(f_ctx, SILOFS_MTYPE_DATA1K, &vaddr);
	if (err) {
		return err;
	}
	flref_setup(out_flref, f_ctx->ii, nullptr, &vaddr, f_ctx->off,
                                        ...
                                        struct silofs_fileaf_ref *out_flref)
{
	...
	err = filc_create_data_leaf(f_ctx, SILOFS_MTYPE_DATA4K, &vaddr);
	if (err) {
		return err;
	}
	flref_setup(out_flref, f_ctx->ii, nullptr, &vaddr, f_ctx->off,
                                 ...
                                 struct silofs_fileaf_ref *flref)
{
	...
	if (flref->tree && flref->has_data && !flref->shared) {
		ret = silofs_test_shared_dbkref(f_ctx->ectx, &flref->vaddr,
		                                &flref->shared);
	}
	return ret;
}
...
{
	return silofs_flush_dirty(f_ctx->ectx, f_ctx->ii, flags);
}
...
int silofs_do_write_iter(struct silofs_exec_ctx *ectx,
                         struct silofs_inode_info *ii, int o_flags,
...
int silofs_do_write(struct silofs_exec_ctx *ectx, struct silofs_inode_info *ii,
...
int silofs_do_rdwr_post(const struct silofs_exec_ctx *ectx, int wr_mode,
...
int silofs_drop_reg(struct silofs_exec_ctx *ectx, struct silofs_inode_info *ii)
{
	struct silofs_file_ctx f_ctx = {
		.ectx     = ectx,
		.env      = ectx->ex_env,
                                        ...
                                        const struct silofs_fileaf_ref *flref)
{
	...
	const size_t len      = len_of_data(pos, f_ctx->end, vaddr->mtype);
	const off_t off_in_bk = off_in_data(pos, vaddr->mtype);
	int err;
...
static int filc_discard_via_unwritten_by(const struct silofs_file_ctx *f_ctx,
                                         const struct silofs_fileaf_ref *flref)
{
	return silofs_mark_unwritten_at(f_ctx->ectx, &flref->vaddr);
}
...
int silofs_do_truncate(struct silofs_exec_ctx *ectx,
...
int silofs_do_lseek(struct silofs_exec_ctx *ectx, struct silofs_inode_info *ii,
...
int silofs_do_fallocate(struct silofs_exec_ctx *ectx,
                                 ...
                                 const struct silofs_vaddr *vaddr)
{
	...
	dsz = len_of_data(f_ctx->off, end, vaddr->mtype);
	if (dsz == 0) {
		return false;
	}
...
int silofs_do_fiemap(struct silofs_exec_ctx *ectx,
                                ...
                                struct silofs_fileaf_ref *flref)
{
	...
	err = filc_claim_data_space(f_ctx, flref->vaddr.mtype,
	                            &flref_new.vaddr);
	if (err) {
		return err;
	}
                                 ...
                                 struct silofs_fileaf_ref *flref)
{
	...
	err = silofs_resolve_llink_of(f_ctx->ectx, &flref->vaddr,
...
int silofs_do_copy_file_range(struct silofs_exec_ctx *ectx,
...
{
	off_t spbh;
	const off_t span    = ftn_span(ftn);
	const size_t height = ftn_height(ftn);
	enum silofs_mtype child_mtype;
	enum silofs_mtype expect_mtype;
	int err;
	...
	child_mtype = ftn_child_mtype(ftn);
	ftn_child_mtype_by_height(ftn, height, &expect_mtype);
