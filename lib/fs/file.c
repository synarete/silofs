...
static void
infl_head1_leaf(const struct silofs_inode_file *infl,
...
static void
infl_set_head1_leaf(struct silofs_inode_file *infl,
                    size_t head1_slot, const struct silofs_vaddr *vaddr)
{
	const size_t slot = infl_head1_slot_to_abs(infl, head1_slot);
...
static void
infl_head2_leaf(const struct silofs_inode_file *infl,
...
static void
infl_set_head2_leaf(struct silofs_inode_file *infl,
                    size_t head2_slot, const struct silofs_vaddr *vaddr)
{
	const size_t slot = infl_head2_slot_to_abs(infl, head2_slot);
...
{
	flref->unwritten = true;
	return vaddr_isnull(&flref->vaddr) ? 0 :
	       silofs_test_unwritten_at(f_ctx->task,
	                                &flref->vaddr, &flref->unwritten);
}
...
static int
filc_seek_data_by_heads(struct silofs_file_ctx *f_ctx,
                        struct silofs_fileaf_ref *out_flref)
{
...
static int
filc_seek_hole_by_heads(struct silofs_file_ctx *f_ctx,
                        struct silofs_fileaf_ref *out_flref)
{
	while (filc_has_head1_leaves_io(f_ctx)) {
		filc_resolve_head1_leaf(f_ctx, out_flref);
...
static int
filc_do_stage_by_tree_from(const struct silofs_file_ctx *f_ctx,
...
static int
filc_resolve_fpos_by_heads(struct silofs_file_ctx *f_ctx,
                           struct silofs_fileaf_ref *out_flref)
{
	int err = 0;
                             ...
                             struct silofs_fileaf_ref *flref_dst, size_t len)
{
	...
	err = filc_copy_data_leaf_by(f_ctx_src, flref_src,
...
static bool filc_test_ismutable_by(const struct silofs_file_ctx *f_ctx,
                                   const struct silofs_fileaf_ref *flref)
{
	return taddr_isnull(&flref->taddr) ||
	       silofs_sbi_ismutable_taddr(f_ctx->sbi, &flref->taddr);
}
...
static int filc_resolve_taddr_by(const struct silofs_file_ctx *f_ctx,
                                 struct silofs_fileaf_ref *flref)
{
	...
	err = silofs_resolve_tlink_of(f_ctx->task, &flref->vaddr,
	                              f_ctx->stg_mode, &tlink);
	if (err) {
		return err;
	}
	taddr_assign(&flref->taddr, &tlink.taddr);
	return 0;
}
                           ...
                           struct silofs_fileaf_ref *flref_dst, size_t len)
{
	...
	if (!flref_src->has_data && flref_dst->has_data) {
		...
		err = filc_resolve_taddr_by(f_ctx_src, flref_src);
		if (err) {
			return err;
		}
		...
		err = filc_resolve_taddr_by(f_ctx_dst, flref_dst);
		if (err) {
			return err;
		}
		if (filc_test_may_share_leaf_by(f_ctx_src, flref_src, 1) &&
