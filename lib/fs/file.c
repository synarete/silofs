                                  ...
                                  const struct silofs_vaddr *vaddr)
{
	...
	return silofs_resolve_llink_of(f_ctx->exct, vaddr, stg_mode, &llink);
}
...
static void filc_update_post_io(const struct silofs_file_ctx *f_ctx)
{
	...
	silofs_update_iattrs_of(f_ctx->exct, ii, &iattr);
}
                                    ...
                                    struct silofs_fileaf_ref *flref)
{
	...
	return silofs_vaddr_isnull(&flref->vaddr) ?
	               0 :
	               silofs_test_unwritten_at(f_ctx->exct, &flref->vaddr,
	                                        &flref->unwritten);
}
                             ...
                             struct silofs_ftleaf_info **out_fli)
{
	...
	err = silofs_stage_vnode(f_ctx->exct, f_ctx->ii, vaddr,
	                         f_ctx->stg_mode, &vni);
	if (err) {
		return err;
	}
...
int silofs_do_read_iter(struct silofs_exec_ctx *exct,
...
int silofs_do_read(struct silofs_exec_ctx *exct, struct silofs_inode_info *ii,
...
static int filc_clear_unwritten_at(const struct silofs_file_ctx *f_ctx,
                                   const struct silofs_vaddr *vaddr)
{
	return silofs_clear_unwritten_at(f_ctx->exct, vaddr);
}
...
filc_claim_vspace(const struct silofs_file_ctx *f_ctx, enum silofs_mtype mtype,
                  struct silofs_vaddr *out_vaddr)
{
	return silofs_claim_vspace(f_ctx->exct, mtype, out_vaddr);
}
...
static int filc_share_data_space(const struct silofs_file_ctx *f_ctx,
                                 const struct silofs_vaddr *vaddr)
{
	return silofs_addref_vspace(f_ctx->exct, vaddr);
}

static int filc_reclaim_data_space(const struct silofs_file_ctx *f_ctx,
                                   const struct silofs_vaddr *vaddr)
{
	return silofs_reclaim_vspace(f_ctx->exct, vaddr);
}
                               ...
                               const struct silofs_vaddr *vaddr)
{
	...
	err = silofs_test_last_allocated(f_ctx->exct, vaddr, &last);
	if (err) {
		return err;
	}
	...
	err = silofs_remove_vnode_at(f_ctx->exct, vaddr);
	if (err) {
		return err;
	}
	return 0;
                             ...
                             struct silofs_ftnode_info **out_fni)
{
	...
	err = silofs_spawn_vnode(f_ctx->exct, f_ctx->ii, SILOFS_MTYPE_FTNODE,
	                         &vni);
	if (err) {
		return err;
	}
...
static int filc_remove_finode(const struct silofs_file_ctx *f_ctx,
                              struct silofs_ftnode_info *fni)
{
	return silofs_remove_vnode(f_ctx->exct, &fni->ftn_vni);
}
...
{
	silofs_update_iblocks_of(f_ctx->exct, f_ctx->ii, vaddr->mtype, dif);
}
                                 ...
                                 struct silofs_fileaf_ref *flref)
{
	...
	if (flref->tree && flref->has_data && !flref->shared) {
		ret = silofs_test_shared_dbkref(f_ctx->exct, &flref->vaddr,
		                                &flref->shared);
	}
	return ret;
}
...
{
	return silofs_flush_dirty(f_ctx->exct, f_ctx->ii, flags);
}
...
int silofs_do_write_iter(struct silofs_exec_ctx *exct,
                         struct silofs_inode_info *ii, int o_flags,
...
int silofs_do_write(struct silofs_exec_ctx *exct, struct silofs_inode_info *ii,
...
int silofs_do_rdwr_post(const struct silofs_exec_ctx *exct, int wr_mode,
...
int silofs_drop_reg(struct silofs_exec_ctx *exct, struct silofs_inode_info *ii)
{
	struct silofs_file_ctx f_ctx = {
		.exct     = exct,
		.env      = exct->env,
...
static int filc_discard_via_unwritten_by(const struct silofs_file_ctx *f_ctx,
                                         const struct silofs_fileaf_ref *flref)
{
	return silofs_mark_unwritten_at(f_ctx->exct, &flref->vaddr);
}
...
int silofs_do_truncate(struct silofs_exec_ctx *exct,
...
int silofs_do_lseek(struct silofs_exec_ctx *exct, struct silofs_inode_info *ii,
...
int silofs_do_fallocate(struct silofs_exec_ctx *exct,
...
int silofs_do_fiemap(struct silofs_exec_ctx *exct,
                                 ...
                                 struct silofs_fileaf_ref *flref)
{
	...
	err = silofs_resolve_llink_of(f_ctx->exct, &flref->vaddr,
...
int silofs_do_copy_file_range(struct silofs_exec_ctx *exct,
