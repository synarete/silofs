...
static ssize_t stype_data_size(enum silofs_stype stype)
{
...
{
	const ssize_t len = stype_data_size(stype);
...
static bool off_inrange(loff_t off, loff_t beg, loff_t end)
{
...
{
	return off_inrange(off, 0, off_head1_max());
}

static bool off_is_head2(loff_t off)
{
	return off_inrange(off, off_head1_max(), off_head2_max());
}
...
{
	return (size_t)stype_data_size(off_to_data_stype(off));
}
...
{
	return off_inrange(pos, ftn_beg(ftn), ftn_end(ftn));
}
                            ...
                            struct silofs_fileaf_info **out_fli)
{
	...
	ii_incref(f_ctx->ii);
	ret = silofs_sbi_stage_vnode(f_ctx->sbi, vaddr, f_ctx->stg_mode, &vi);
	if (ret) {
		goto out;
	}
	fli = silofs_fli_from_vi(vi);
	silofs_fli_rebind_view(fli);
	ret = fic_recheck_fileaf(f_ctx, fli);
	if (ret) {
		goto out;
	}
	*out_fli = fli;
...
static int fic_zero_data_leaf_at(const struct silofs_file_ctx *f_ctx,
                                 const struct silofs_vaddr *vaddr)
{
	const ssize_t len = stype_data_size(vaddr_stype(vaddr));
                            ...
                            struct silofs_finode_info **out_fni)
{
	...
	ret = fic_recheck_finode(f_ctx, fni);
	if (ret) {
		goto out;
	}
	*out_fni = fni;
                                   ...
                                   struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	if (fni_isbottom(parent_fni)) {
		ret = fic_seek_tree_at_leaves(f_ctx, parent_fni, out_fm_ctx);
		goto out;
	}
                                   ...
                                   struct silofs_fileaf_info *fli, size_t *sz)
{
	...
	fli_incref(fli);
	ret = fic_export_data_by_fileaf(f_ctx, fli, sz);
	fli_decref(fli);
                                ...
                                struct silofs_finode_info *parent_fni)
{
	...
	while (fic_has_more_io(f_ctx)) {
		...
		ret = fic_read_from_leaf(f_ctx, &fm_ctx, &len);
		if (ret) {
			break;
		}
                              ...
                              struct silofs_finode_info **out_fni)
{
	...
	if (fm_ctx.has_target) {
		ret = fic_stage_finode(f_ctx, &fm_ctx.vaddr, out_fni);
	} else {
...
static int fic_write_leaf_by_copy(const struct silofs_file_ctx *f_ctx,
                                  struct silofs_fileaf_info *fli, size_t *sz)
{
	int ret;

	fli_incref(fli);
	ret = fic_import_data_by_fileaf(f_ctx, fli, sz);
	if (ret) {
		goto out;
	}
	ret = fic_clear_unwritten_of(f_ctx, fli);
	if (ret) {
		goto out;
	}
out:
                               ...
                               struct silofs_finode_info *parent_fni)
{
	...
	fni_incref(parent_fni);
	while (fic_has_more_io(f_ctx)) {
		ret = fic_require_tree_leaf(f_ctx, parent_fni, &fm_ctx);
		if (ret) {
			break;
		}
		ret = fmc_write_to_leaf(&fm_ctx, &len);
		if (ret) {
			break;
		}
		fic_advance_by_nbytes(f_ctx, len);
...
static int fmc_discard_partial(const struct silofs_fmap_ctx *fm_ctx)
{
	...
	err = fic_zero_data_leaf_range(fm_ctx->f_ctx, vaddr, blobid, len);
...
static int fic_truncate(struct silofs_file_ctx *f_ctx)
{
	...
	ret = fic_resolve_truncate_end(f_ctx);
	if (ret) {
		goto out;
	}
	ret = fic_discard_data(f_ctx);
	if (ret) {
		goto out;
	}
	ret = fic_discard_unused_meta(f_ctx);
	if (ret) {
		goto out;
	}
	fic_update_post_io(f_ctx, ret == 0);
out:
	ii_decref(f_ctx->ii);
	return ret;
}
...
static int fic_lseek(struct silofs_file_ctx *f_ctx)
{
	...
	if (fic_is_seek_data(f_ctx)) {
		ret = fic_lseek_data(f_ctx);
	} else if (fic_is_seek_hole(f_ctx)) {
		ret =  fic_lseek_hole(f_ctx);
	} else {
		ret = fic_lseek_notsupp(f_ctx);
	}
out:
	ii_decref(f_ctx->ii);
	return ret;
...
static int fic_fallocate(struct silofs_file_ctx *f_ctx)
{
	...
	ret = fic_check_fl_mode(f_ctx);
	if (ret) {
		goto out;
	}
	ret = fic_fallocate_op(f_ctx);
	if (ret) {
		goto out;
	}
	fic_update_post_io(f_ctx, false);
                     ...
                     struct silofs_inode_info *ii, struct fiemap *fm)
{
	...
	return fic_fiemap(&f_ctx);
}

/*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/

static int
fic_resolve_fmap_recursive(struct silofs_file_ctx *f_ctx,
                           struct silofs_finode_info *parent_fni,
                           struct silofs_fmap_ctx *out_fm_ctx);
                              ...
                              size_t slot, struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_resolve_fmap_recursive(f_ctx, fni, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
...
static int fic_resolve_fmap_recursive(struct silofs_file_ctx *f_ctx,
                                      struct silofs_finode_info *parent_fni,
                                      struct silofs_fmap_ctx *out_fm_ctx)
{
	size_t slot;
	...
	if (!fni_isbottom(parent_fni)) {
		ret = fic_resolve_fmap_recursive_at(f_ctx, parent_fni,
		                                    slot, out_fm_ctx);
		goto out;
	}
                                        ...
                                        struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_resolve_fmap_recursive(f_ctx, root_fni, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
                          ...
                          struct silofs_file_ctx *f_ctx_dst, size_t *out_ncp)
{
	...
	ii_incref(f_ctx_src->ii);
	ii_incref(f_ctx_dst->ii);
	ret = fic_check_copy_range(f_ctx_src, f_ctx_dst);
	if (ret) {
		goto out;
	}
	ret = fic_copy_range_iter(f_ctx_src, f_ctx_dst);
	if (ret) {
		goto out;
	}
...
out:
	ii_decref(f_ctx_dst->ii);
	ii_decref(f_ctx_src->ii);
	return ret;
}
