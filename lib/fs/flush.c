...
static int uber_flush_warm(struct silofs_fs_uber *uber, int flags)
{
	struct silofs_flush_ctx fl_ctx = {
		.uber = uber,
		.repo = &uber->ub_repos->repo_warm,
		.cache = &uber->ub_repos->repo_warm.re_cache,
		.flags = flags,
		.warm = true,
	};
	int err;
...
int silofs_uber_flush_dirty(struct silofs_fs_uber *uber, int flags)
{
	...
	err = uber_flush_warm(uber, flags);
	if (err) {
		return err;
	}
