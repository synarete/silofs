...
static void guarantee_persistent_types_members(void)
{
	...
	REQUIRE_MEMBER_SIZE(struct silofs_bk_ref, br_reserved, 56);
...
static void guarantee_persistent_types_alignment1(void)
{
	...
	REQUIRE_OFFSET64(struct silofs_bk_ref, br_allocated, 40);
	REQUIRE_OFFSET64(struct silofs_bk_ref, br_unwritten, 48);
	REQUIRE_OFFSET64(struct silofs_bk_ref, br_refcnt, 56);
}
...
static void guarantee_persistent_types_alignment2(void)
{
	...
	REQUIRE_OFFSET64(struct silofs_spmap_node, sn_subref, 512);
	...
	REQUIRE_OFFSET64(struct silofs_spmap_leaf, sl_subref, 512);
}
...
static void guarantee_defs_consistency(void)
{
	...
	REQUIRE_EQ(SILOFS_BLOB_SIZE_MAX, 64 * SILOFS_MEGA);
	REQUIRE_LE(SILOFS_BK_SIZE * SILOFS_SPMAP_NCHILDS,
	...
	REQUIRE_EQ(SILOFS_VSPACE_SIZE_MAX, 4 * SILOFS_PETA);
