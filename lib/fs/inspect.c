...
static bool spvis_unique_blobid(const struct silofs_sp_visitor *sp_vis,
                                const struct silofs_blobid *blobid, size_t cnt)
{
	if (blobid_isnull(blobid)) {
		return false;
	}
	for (size_t i = 0; i < cnt; ++i) {
		if (blobid_isequal(blobid, &sp_vis->urefs.subs[i].blobid)) {
			return false;
		}
	}
	return true;
                     ...
                     const struct silofs_spleaf_info *sli, loff_t voff)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(sp_vis->urefs.subs); ++i) {
		paddr = &sp_vis->urefs.subs[i];
		if (spvis_unique_blobid(sp_vis, &paddr->blobid, i)) {
			sp_vis->cb(paddr, voff);
		}
                         ...
                         const struct silofs_walk_iter *witr)
{
	...
	case SILOFS_HEIGHT_SPNODE4:
		sp_vis->cb(sni_paddr(witr->sni4), witr->voff);
		sp_vis->sp_st.objs.nspnode++;
		break;
	case SILOFS_HEIGHT_SPNODE3:
		sp_vis->cb(sni_paddr(witr->sni3), witr->voff);
		sp_vis->sp_st.objs.nspnode++;
		break;
	case SILOFS_HEIGHT_SPNODE2:
		sp_vis->cb(sni_paddr(witr->sni2), witr->voff);
		sp_vis->sp_st.objs.nspnode++;
		break;
	case SILOFS_HEIGHT_SPNODE1:
		sp_vis->cb(sni_paddr(witr->sni1), witr->voff);
		sp_vis->sp_st.objs.nspnode++;
		break;
	case SILOFS_HEIGHT_SPLEAF:
		sp_vis->cb(sli_paddr(witr->sli), witr->voff);
...
static void noop_callback(const struct silofs_paddr *paddr, loff_t voff)
{
	silofs_unused(paddr);
	silofs_unused(voff);
}
...
spvis_new(struct silofs_alloc *alloc, silofs_visit_paddr_fn cb)
{
