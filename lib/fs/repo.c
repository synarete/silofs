                     ...
                     const struct silofs_blobid *blobid)
{
	...
	bli->bl_hkey = silofs_blobid_as_u64(blobid);
...
{
	struct silofs_blobid40b blid;
	const struct silofs_mdigest *md = repo_mdigest(repo);

	silofs_blobid40b_set(&blid, blobid);
	silofs_sha256_of(md, &blid, sizeof(blid), out_hash);
}
                                  ...
                                  struct silofs_ubk_info **out_ubki)
{
	...
	if (!err) {
		silofs_ubki_attach(*out_ubki, bli);
	}
	bli_decref(bli);
	return err;
}
                          ...
                          struct silofs_ubk_info **out_ubki)
{
	...
	err = repo_spawn_attach_ubki(repo, bli, bkaddr, out_ubki);
	if (err) {
		return err;
	}
	return 0;
                          ...
                          struct silofs_ubk_info **out_ubki)
{
	...
	err = repo_spawn_attach_ubki(repo, bli, bkaddr, &ubki);
	if (err) {
		return err;
	}
	err = silofs_bli_load_bk(bli, bkaddr, ubki->ubk);
	if (err) {
