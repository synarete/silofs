...
static int stgc_do_stage_lseg(const struct silofs_stage_ctx *stg_ctx,
                              const struct silofs_lsegid *lsegid)
{
	int err = -SILOFS_ENOMEM;

	for (size_t i = 0; i < stg_ctx->retry; ++i) {
		err = silofs_stage_lseg_at(stg_ctx->fsenv, lsegid);
		if (!is_low_resource_error(err)) {
			break;
		}
		stgc_try_evict_some(stg_ctx, i > 0);
...
static int stgc_do_stage_lseg_of(const struct silofs_stage_ctx *stg_ctx,
                                 const struct silofs_bkaddr *bkaddr)
{
	return stgc_do_stage_lseg(stg_ctx, &bkaddr->laddr.lsegid);
}

/*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/

static int stgc_spawn_vi_at(const struct silofs_stage_ctx *stg_ctx,
                            const struct silofs_llink *llink,
                            struct silofs_vnode_info **out_vi)
{
	int err;

	err = stgc_do_spawn_vi(stg_ctx, stg_ctx->vaddr, out_vi);
	if (err) {
		return err;
	}
	vi_bind_to(*out_vi, stg_ctx->fsenv);
                                ...
                                struct silofs_vnode_info *vi)
{
	...
	err = silofs_refresh_llink_of(stg_ctx->task, vi);
	if (err) {
		return err;
	}
	err = silofs_decrypt_vi_view(stg_ctx->fsenv, vi);
	if (err) {
		return err;
	}
...
static int stgc_do_spawn_lseg(const struct silofs_stage_ctx *stg_ctx,
                              const struct silofs_lsegid *lsegid)
{
	return silofs_spawn_lseg_at(stg_ctx->fsenv, lsegid);
}

static int stgc_spawn_lseg(const struct silofs_stage_ctx *stg_ctx,
                           const struct silofs_lsegid *lsegid,
                           enum silofs_stype stype_sub)
{
	int err;

	err = stgc_do_spawn_lseg(stg_ctx, lsegid);
	if (!err) {
		silofs_sti_update_lsegs(&stg_ctx->sbi->sb_sti, stype_sub, 1);
	}
	return err;
}

static void
stgc_make_lsegid_of_spmaps(const struct silofs_stage_ctx *stg_ctx,
                           ...
                           struct silofs_lsegid *out_lsegid)
{
	...
	silofs_sbi_get_lvid(stg_ctx->sbi, &lvid);
	silofs_lsegid_setup(out_lsegid, &lvid, voff, vspace, height);
}

static void
stgc_make_lsegid_of_vdata(const struct silofs_stage_ctx *stg_ctx,
...
static int stgc_spawn_super_main_lseg(const struct silofs_stage_ctx *stg_ctx)
{
	struct silofs_lsegid lsegid;
	const enum silofs_height height = SILOFS_HEIGHT_SUPER - 1;
	int err;

	stgc_make_lsegid_of_spmaps(stg_ctx, 0, height, &lsegid);
	err = stgc_spawn_lseg(stg_ctx, &lsegid, SILOFS_STYPE_SPNODE);
	if (err) {
		return err;
	}
	silofs_sbi_bind_main_lseg(stg_ctx->sbi, stg_ctx->vspace, &lsegid);
	return 0;
}

static int stgc_stage_super_main_lseg(const struct silofs_stage_ctx *stg_ctx)
{
	struct silofs_lsegid lsegid;

	silofs_sbi_main_lseg(stg_ctx->sbi, stg_ctx->vspace, &lsegid);
	return stgc_do_stage_lseg(stg_ctx, &lsegid);
}

static int stgc_require_super_main_lseg(const struct silofs_stage_ctx *stg_ctx)
{
	int err;

	if (silofs_sbi_has_main_lseg(stg_ctx->sbi, stg_ctx->vspace)) {
		err = stgc_stage_super_main_lseg(stg_ctx);
	} else {
		err = stgc_spawn_super_main_lseg(stg_ctx);
	}
	return err;
}
...
static int stgc_spawn_spnode_main_lseg(const struct silofs_stage_ctx *stg_ctx,
                                       struct silofs_spnode_info *sni)
{
	...
	stgc_make_lsegid_of_spmaps(stg_ctx, voff, height, &lsegid);
	err = stgc_spawn_lseg(stg_ctx, &lsegid, stype);
	if (err) {
		return err;
	}
...
static int stgc_stage_spnode_main_lseg(const struct silofs_stage_ctx *stg_ctx,
                                       struct silofs_spnode_info *sni)
{
	struct silofs_lsegid lsegid;
...
static int
stgc_require_spnode_main_lseg(const struct silofs_stage_ctx *stg_ctx,
                              struct silofs_spnode_info *sni)
{
	int err;

	if (sni_has_main_lseg(sni)) {
		err = stgc_stage_spnode_main_lseg(stg_ctx, sni);
	} else {
		err = stgc_spawn_spnode_main_lseg(stg_ctx, sni);
	}
	return err;
}
...
static loff_t stgc_lbk_voff(const struct silofs_stage_ctx *stg_ctx)
{
	return off_align_to_lbk(stg_ctx->voff);
}
                                  ...
                                  struct silofs_unode_info **out_ui)
{
	...
	silofs_vrange_of_spmap(&vrange, height, stgc_lbk_voff(stg_ctx));
...
{
	return silofs_sni_resolve_child(sni, stgc_lbk_voff(stg_ctx),
	                                out_ulink);
}
...
{
	return silofs_sli_resolve_child(sli, stgc_lbk_voff(stg_ctx),
	                                out_blink);
}
                                 ...
                                 struct silofs_spnode_info **out_sni)
{
	...
	err = stgc_require_super_main_lseg(stg_ctx);
	if (err) {
		return err;
	}
                                 ...
                                 struct silofs_spnode_info **out_sni)
{
	...
	err = stgc_require_spnode_main_lseg(stg_ctx, stg_ctx->sni4);
	if (err) {
		return err;
	}
	silofs_sni_resolve_main(stg_ctx->sni4, stgc_lbk_voff(stg_ctx), &ulink);
...
{
	return sni_has_child_at(stg_ctx->sni4, stgc_lbk_voff(stg_ctx));
}
                                 ...
                                 struct silofs_spnode_info **out_sni)
{
	...
	err = stgc_require_spnode_main_lseg(stg_ctx, stg_ctx->sni3);
	if (err) {
		return err;
	}
	silofs_sni_resolve_main(stg_ctx->sni3, stgc_lbk_voff(stg_ctx), &ulink);
...
{
	return sni_has_child_at(stg_ctx->sni3, stgc_lbk_voff(stg_ctx));
}
                                 ...
                                 struct silofs_spnode_info **out_sni)
{
	...
	err = stgc_require_spnode_main_lseg(stg_ctx, stg_ctx->sni2);
	if (err) {
		return err;
	}
	silofs_sni_resolve_main(stg_ctx->sni2, stgc_lbk_voff(stg_ctx), &ulink);
...
{
	return sni_has_child_at(stg_ctx->sni2, stgc_lbk_voff(stg_ctx));
}
                                ...
                                struct silofs_spleaf_info **out_sli)
{
	...
	err = stgc_require_spnode_main_lseg(stg_ctx, stg_ctx->sni1);
	if (err) {
		return err;
	}
	silofs_sni_resolve_main(stg_ctx->sni1, stgc_lbk_voff(stg_ctx), &ulink);
...
static int
stgc_require_spleaf_main_lseg(const struct silofs_stage_ctx *stg_ctx,
                              struct silofs_spleaf_info *sli)
{
	struct silofs_lsegid lsegid;
	int err;
	 ...
	 */
	stgc_make_lsegid_of_vdata(stg_ctx, sli_base_voff(sli), &lsegid);
	err = stgc_do_stage_lseg(stg_ctx, &lsegid);
	if (!err) {
		goto out_ok;
	}
	...
	err = stgc_spawn_lseg(stg_ctx, &lsegid, stg_ctx->vspace);
	if (err) {
		return err;
	}
out_ok:
                             ...
                             struct silofs_spleaf_info **out_sli)
{
	...
	err = stgc_require_spleaf_main_lseg(stg_ctx, *out_sli);
	if (err) {
		return err;
	}
...
{
	return sni_has_child_at(stg_ctx->sni1, stgc_lbk_voff(stg_ctx));
}
                                 ...
                                 struct silofs_llink *out_llink)
{
	...
	silofs_laddr_setup(&laddr, &blink.bka.laddr.lsegid,
...
static int stgc_load_view_at(const struct silofs_stage_ctx *stg_ctx,
                             const struct silofs_laddr *laddr,
                             struct silofs_view *view)
{
	struct silofs_repo *repo = stg_ctx->fsenv->fse.repo;
	int ret = 0;
	bool raw;
...
static int stgc_do_require_lseg_of(const struct silofs_stage_ctx *stg_ctx,
                                   const struct silofs_laddr *laddr)
{
	int err;
	...
	err = stgc_do_stage_lseg(stg_ctx, &laddr->lsegid);
	if (err) {
		return err;
	}
	return 0;
...
static int stgc_stage_load_view(const struct silofs_stage_ctx *stg_ctx,
                                const struct silofs_laddr *laddr,
                                struct silofs_view *view)
{
	int err;
	...
	err = stgc_load_view_at(stg_ctx, laddr, view);
	if (err) {
		return err;
	}
	return 0;
                            ...
                            const struct silofs_bkaddr *bkaddr_dst)
{
	...
	ret = stgc_do_stage_lseg_of(stg_ctx, bkaddr_src);
	if (ret) {
		goto out;
	}
	ret = stgc_do_stage_lseg_of(stg_ctx, bkaddr_dst);
	if (ret) {
		goto out;
	}
                                     ...
                                     struct silofs_blink *out_blink_dst)
{
	...
	err = stgc_require_spleaf_main_lseg(stg_ctx, stg_ctx->sli);
	if (err) {
		return err;
	}
...
static int stgc_clone_rebind_lbk(const struct silofs_stage_ctx *stg_ctx,
...
static int stgc_require_lseg_of(const struct silofs_stage_ctx *stg_ctx,
                                const struct silofs_laddr *laddr)
{
	const struct silofs_vaddr *vaddr = stg_ctx->vaddr;
	int ret = 0;
	...
	if (!stype_isdata(vaddr->stype)) {
		ret = stgc_do_require_lseg_of(stg_ctx, laddr);
...
static int stgc_pre_clone_lbk(struct silofs_stage_ctx *stg_ctx,
                              struct silofs_vis *vis)
{
	int err;
...
static void stgc_post_clone_lbk(const struct silofs_stage_ctx *stg_ctx,
...
static int stgc_clone_lbk_at(struct silofs_stage_ctx *stg_ctx,
                             const struct silofs_bkaddr *src_bka)
{
	struct silofs_vis vis = { .vas.count = 0 };
	int err;

	err = stgc_pre_clone_lbk(stg_ctx, &vis);
	if (!err) {
		err = stgc_clone_rebind_lbk(stg_ctx, src_bka);
	}
	stgc_post_clone_lbk(stg_ctx, &vis);
	return err;
}

static int stgc_clone_lbk_of(struct silofs_stage_ctx *stg_ctx,
                             const struct silofs_laddr *src_laddr)
{
	struct silofs_bkaddr src_bka = { .lba = SILOFS_LBA_NULL };

	bkaddr_by_laddr(&src_bka, src_laddr);
	return stgc_clone_lbk_at(stg_ctx, &src_bka);
}
                                      ...
                                      struct silofs_llink *out_llink)
{
	...
	err = stgc_require_lseg_of(stg_ctx, &out_llink->laddr);
	if (err) {
		return err;
	}
	err = stgc_clone_lbk_of(stg_ctx, &out_llink->laddr);
	if (err) {
		return err;
	}
	err = stgc_resolve_llink_of(stg_ctx, out_llink);
...
static int stgc_fetch_cached_vnode(const struct silofs_stage_ctx *stg_ctx,
                                   struct silofs_vnode_info **out_vi)
{
	return silofs_fetch_cached_vnode(stg_ctx->task,
	                                 stg_ctx->vaddr, out_vi);
}
                               ...
                               struct silofs_vnode_info **out_vi)
{
	...
	err = stgc_do_require_lseg_of(stg_ctx, &llink.laddr);
	if (err) {
		goto out_err;
	}
	err = stgc_fetch_cached_vnode(stg_ctx, &vi);
	if (!err) {
		/* vnode cached upon inspect -- just update llink */
		goto out_ok;
	}
	err = stgc_spawn_vi_at(stg_ctx, &llink, &vi);
	if (err) {
		goto out_err;
	}
