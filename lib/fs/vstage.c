...
{
	silofs_relax_caches(vstg_ctx->task, SILOFS_F_NOW);
}
                               ...
                               const struct silofs_lsegid *lsegid)
{
	...
	for (size_t i = 0; i < vstg_ctx->retry; ++i) {
		err = silofs_stage_lseg_at(vstg_ctx->fsenv, lsegid);
		if (!is_low_resource_error(err)) {
			break;
		}
		vstgc_try_evict_some(vstg_ctx, i > 0);
...
{
	return vstgc_do_stage_lseg(vstg_ctx, &bkaddr->laddr.lsegid);
}
...
static int vstgc_do_spawn_lseg(const struct silofs_vstage_ctx *vstg_ctx,
                               const struct silofs_lsegid *lsegid)
{
	return silofs_spawn_lseg_at(vstg_ctx->fsenv, lsegid);
}
                            ...
                            enum silofs_stype stype_sub)
{
	...
	if (!err) {
		silofs_sti_update_lsegs(&vstg_ctx->sbi->sb_sti, stype_sub, 1);
	}
	return err;
}
                            ...
                            struct silofs_lsegid *out_lsegid)
{
	...
	const enum silofs_stype vspace = vstg_ctx->vspace;
...
{
	silofs_sti_update_objs(&vstg_ctx->sbi->sb_sti, uaddr->stype, 1);
	silofs_sti_update_bks(&vstg_ctx->sbi->sb_sti, uaddr->stype, 1);
}
                                       ...
                                       *vstg_ctx)
{
	...
	err = vstgc_spawn_lseg(vstg_ctx, &lsegid, SILOFS_STYPE_SPNODE);
	if (err) {
		return err;
	}
                                        ...
                                        struct silofs_spnode_info *sni)
{
	...
	err = vstgc_spawn_lseg(vstg_ctx, &lsegid, stype);
	if (err) {
		return err;
	}
...
{
	return silofs_sni_resolve_child(sni, vstgc_lbk_voff(vstg_ctx),
	                                out_ulink);
}
...
{
	return silofs_sli_resolve_child(sli, vstgc_lbk_voff(vstg_ctx),
	                                out_blink);
}
                                    ...
                                    struct silofs_spnode_info **out_sni)
{
	...
	for (size_t i = 0; i < vstg_ctx->retry; ++i) {
		err = silofs_stage_spnode_at(vstg_ctx->fsenv, ulink, out_sni);
		if (!is_low_resource_error(err)) {
			break;
		}
		vstgc_try_evict_some(vstg_ctx, i > 0);
                                    ...
                                    struct silofs_spnode_info **out_sni)
{
	...
	for (size_t i = 0; i < vstg_ctx->retry; ++i) {
		err = silofs_spawn_spnode_at(vstg_ctx->fsenv, ulink, out_sni);
		if (!is_low_resource_error(err)) {
			break;
		}
		vstgc_try_evict_some(vstg_ctx, i > 0);
                                    ...
                                    struct silofs_spleaf_info **out_sli)
{
	...
	for (size_t i = 0; i < vstg_ctx->retry; ++i) {
		err = silofs_stage_spleaf_at(vstg_ctx->fsenv, ulink, out_sli);
		if (!is_low_resource_error(err)) {
			break;
		}
		vstgc_try_evict_some(vstg_ctx, i > 0);
                                    ...
                                    struct silofs_spleaf_info **out_sli)
{
	...
	for (size_t i = 0; i < vstg_ctx->retry; ++i) {
		err = silofs_spawn_spleaf_at(vstg_ctx->fsenv, ulink, out_sli);
		if (!is_low_resource_error(err)) {
			break;
		}
		vstgc_try_evict_some(vstg_ctx, i > 0);
                                  ...
                                  struct silofs_llink *out_llink)
{
	...
	err = vstgc_resolve_spleaf_child(vstg_ctx, vstg_ctx->sli, &blink);
	if (err) {
		return err;
	}
...
int silofs_stage_spnode1(struct silofs_task *task,
                         const struct silofs_vaddr *vaddr,
                         enum silofs_stg_mode stg_mode,
                         struct silofs_spnode_info **out_sni)
{
...
int silofs_stage_spmaps(struct silofs_task *task,
                        const struct silofs_vaddr *vaddr,
                        enum silofs_stg_mode stg_mode,
                        struct silofs_spnode_info **out_sni,
                        struct silofs_spleaf_info **out_sli)
...
int silofs_stage_spleaf(struct silofs_task *task,
                        const struct silofs_vaddr *vaddr,
                        enum silofs_stg_mode stg_mode,
                        struct silofs_spleaf_info **out_sli)
{
	...
	return silofs_stage_spmaps(task, vaddr, stg_mode, &sni, out_sli);
}

int silofs_require_spmaps(struct silofs_task *task,
                          const struct silofs_vaddr *vaddr,
                          enum silofs_stg_mode stg_mode,
                          struct silofs_spnode_info **out_sni,
                          struct silofs_spleaf_info **out_sli)
...
static int vstgc_require_bkaddr(const struct silofs_vstage_ctx *vstg_ctx,
                                const struct silofs_bkaddr *bkaddr)
{
	return vstgc_require_laddr(vstg_ctx, &bkaddr->laddr);
}
...
static int vstgc_require_bks(const struct silofs_vstage_ctx *vstg_ctx,
                             const struct silofs_bkaddr *bkaddr_src,
                             const struct silofs_bkaddr *bkaddr_dst)
{
	int ret;

	ret = vstgc_do_stage_lseg_of(vstg_ctx, bkaddr_src);
	if (ret) {
		goto out;
	}
	ret = vstgc_do_stage_lseg_of(vstg_ctx, bkaddr_dst);
	if (ret) {
		goto out;
	}
	ret = vstgc_require_bkaddr(vstg_ctx, bkaddr_src);
	if (ret) {
		goto out;
	}
	ret = vstgc_require_bkaddr(vstg_ctx, bkaddr_dst);
	if (ret) {
		goto out;
	}
out:
...
static int vstgc_require_clone_bkaddr(const struct silofs_vstage_ctx *vstg_ctx,
                                      struct silofs_blink *out_blink_dst)
{
	...
	silofs_sli_resolve_main(vstg_ctx->sli, vaddr->off, out_blink_dst);
	return 0;
}
                                  ...
                                  const struct silofs_bkaddr *src_bka)
{
	...
	err = vstgc_require_clone_bkaddr(vstg_ctx, &dst_blink);
	if (err) {
		return err;
	}
	err = vstgc_require_bks(vstg_ctx, src_bka, &dst_blink.bka);
	if (err) {
		return err;
	}
	silofs_sli_bind_child(vstg_ctx->sli, vstg_ctx->vaddr->off, &dst_blink);
	return 0;
}
                                     ...
                                     struct silofs_vis *vis)
{
	...
	silofs_sli_vaddrs_at(vstg_ctx->sli, vaddr->stype, lba, &vis->vas);
                              ...
                              const struct silofs_bkaddr *src_bka)
{
	...
	if (!err) {
		err = vstgc_clone_rebind_lbk(vstg_ctx, src_bka);
	}
                              ...
                              const struct silofs_laddr *src_laddr)
{
	...
	return vstgc_clone_lbk_at(vstg_ctx, &src_bka);
}
...
int silofs_resolve_llink(struct silofs_task *task,
                         const struct silofs_vaddr *vaddr,
                         enum silofs_stg_mode stg_mode,
                         struct silofs_llink *out_llink)
{
                             ...
                             const struct silofs_vaddr *vaddr)
{
	...
	return silofs_resolve_llink(task, vaddr, SILOFS_STG_COW, &llink);
}
                                     ...
                                     enum silofs_stg_mode stg_mode)
{
	...
	err = silofs_resolve_llink(task, vi_vaddr(vi), stg_mode, &llink);
	if (err) {
		return err;
	}
	vi_update_llink(vi, &llink);
                         ...
                         struct silofs_vnode_info *vi)
{
	...
	vaddr = vi_vaddr(vi);
	err = silofs_resolve_llink(task, vaddr, SILOFS_STG_CUR, &llink);
	if (err) {
