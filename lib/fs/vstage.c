...
static int vstgc_flush_dirty_now(const struct silofs_vstage_ctx *vstg_ctx)
{
	...
	err = silofs_flush_dirty_now(vstg_ctx->exct);
	if (err) {
                                ...
                                struct silofs_vnode_info *vni)
{
	...
	err = silofs_refresh_llink(vstg_ctx->exct, vni);
	if (err) {
		return err;
	}
...
static void
vstgc_setup(struct silofs_vstage_ctx *vstg_ctx, struct silofs_exec_ctx *exct,
            const struct silofs_vaddr *vaddr, enum silofs_stg_mode stg_mode)
{
	memset(vstg_ctx, 0, sizeof(*vstg_ctx));
	vstg_ctx->exct     = exct;
	vstg_ctx->env      = exct->env;
	vstg_ctx->sbi      = exct->env->sbi;
...
static int vstgc_stage_lsmap_for_resolve(struct silofs_vstage_ctx *vstg_ctx)
{
	...
	err = silofs_resolve_llink_of(vstg_ctx->exct, &vaddr,
...
int silofs_stage_spleaf_of(struct silofs_exec_ctx *exct,
                           const struct silofs_vaddr *vaddr,
                           enum silofs_stg_mode stg_mode,
                           struct silofs_spleaf_info **out_sli)
{
	...
	vstgc_setup(&vstg_ctx, exct, vaddr, stg_mode);
...
int silofs_require_spleaf_of(struct silofs_exec_ctx *exct,
                             const struct silofs_vaddr *vaddr,
                             enum silofs_stg_mode stg_mode,
                             struct silofs_spleaf_info **out_sli)
{
...
static int require_stable_at(struct silofs_exec_ctx *exct,
                             const struct silofs_vaddr *vaddr)
{
	struct silofs_vstage_ctx vstg_ctx;
	int err;

	vstgc_setup(&vstg_ctx, exct, vaddr, SILOFS_STG_CUR | SILOFS_STG_RAW);
...
static int
check_stable_at(struct silofs_exec_ctx *exct, const struct silofs_vaddr *vaddr)
{
	struct silofs_vstage_ctx vstg_ctx;
	int err;
                               ...
                               struct silofs_vnode_info **out_vni)
{
	...
	*out_vni = nullptr;
	...
	err = silofs_stage_inode(vstg_ctx->exct, ino, SILOFS_STG_CUR, &ii);
	if (err) {
		return err;
	}
	*out_vni = &ii->i_vni;
                               ...
                               struct silofs_vnode_info **out_vni)
{
	...
	*out_vni = nullptr;
	err      = silofs_stage_vnode(vstg_ctx->exct, nullptr, vaddr,
	                              SILOFS_STG_CUR, &vni);
	if (err) {
...
{
	return silofs_require_lsmap_by(vstg_ctx->exct, vstg_ctx->vaddr,
	                               &vstg_ctx->lsi);
}
...
int silofs_resolve_llink_of(struct silofs_exec_ctx *exct,
                            const struct silofs_vaddr *vaddr,
                            enum silofs_stg_mode stg_mode,
                            struct silofs_llink *out_llink)
{
...
static int vstgc_fetch_cached_vnode(const struct silofs_vstage_ctx *vstg_ctx,
                                    struct silofs_vnode_info **out_vni)
{
	return silofs_fetch_cached_vnode(vstg_ctx->exct, vstg_ctx->vaddr,
	                                 out_vni);
}
 ...
 */
static int fixup_cached_vni(const struct silofs_exec_ctx *exct,
                            struct silofs_vnode_info *vni)
{
	...
	silofs_lcache_forget_vni(exct->lcache, vni);
	return -SILOFS_ENOENT;
}

static int fetch_cached_vni(struct silofs_exec_ctx *exct,
                            const struct silofs_vaddr *vaddr,
                            struct silofs_vnode_info **out_vni)
{
	struct silofs_vnode_info *vni;
	...
	vni = silofs_lcache_lookup_vni(exct->lcache, vaddr);
	if (vni == nullptr) {
		return -SILOFS_ENOENT;
	}
	err = fixup_cached_vni(exct, vni);
	if (err) {
		return err;
	}
	*out_vni = vni;
...
int silofs_fetch_cached_vnode(struct silofs_exec_ctx *exct,
                              const struct silofs_vaddr *vaddr,
                              struct silofs_vnode_info **out_vni)
{
	...
	if (!silofs_vaddr_isnull(vaddr)) {
		ret = fetch_cached_vni(exct, vaddr, out_vni);
	}
	return ret;
}

static int
stage_vnode_at(struct silofs_exec_ctx *exct, const struct silofs_vaddr *vaddr,
               enum silofs_stg_mode stg_mode,
               struct silofs_vnode_info **out_vni)
{
	struct silofs_vstage_ctx vstg_ctx;
...
static int stage_stable_vnode_at(struct silofs_exec_ctx *exct,
                                 const struct silofs_vaddr *vaddr,
                                 enum silofs_stg_mode stg_mode,
                                 struct silofs_vnode_info **out_vni)
{
	...
	err = require_stable_at(exct, vaddr);
	if (err) {
		return err;
	}
	err = stage_vnode_at(exct, vaddr, stg_mode, out_vni);
	if (err) {
		return err;
	}
	return 0;
...
static bool has_mutable_laddr(const struct silofs_exec_ctx *exct,
                              const struct silofs_vnode_info *vni)
{
	...
	if (!silofs_laddr_isnull(laddr)) {
		ret = silofs_sbi_ismutable_laddr(exct->env->sbi, laddr);
	}
	return ret;
}

static int require_updated_cached_vni(struct silofs_exec_ctx *exct,
                                      struct silofs_vnode_info *vni,
                                      enum silofs_stg_mode stg_mode)
{
	struct silofs_llink llink        = { .laddr.pos = -1 };
	...
	if (has_mutable_laddr(exct, vni)) {
		return 0;
	}
	vaddr = silofs_vni_vaddr(vni);
	err   = silofs_resolve_llink_of(exct, vaddr, stg_mode, &llink);
	if (err) {
		return err;
	}
	vni_update_llink(vni, &llink);
...
static int do_resolve_stage_vnode(struct silofs_exec_ctx *exct,
                                  const struct silofs_vaddr *vaddr,
                                  enum silofs_stg_mode stg_mode,
                                  struct silofs_vnode_info **out_vni)
{
	...
	err = fetch_cached_vni(exct, vaddr, out_vni);
	if (!err) {
		/* cache hit -- require up-to-date */
		err = require_updated_cached_vni(exct, *out_vni, stg_mode);
	} else {
		/* cache miss -- stage from objects store */
		err = stage_stable_vnode_at(exct, vaddr, stg_mode, out_vni);
	}
	return err;
}

static int check_stage_mode(const struct silofs_exec_ctx *exct,
                            enum silofs_stg_mode stg_mode)
{
	int ret = 0;

	if (stg_mode & SILOFS_STG_COW) {
		ret = silofs_env_isrdonlyfs(exct->env) ? -SILOFS_EROFS : 0;
	}
	return ret;
}

static int check_stage_vnode(const struct silofs_exec_ctx *exct,
                             const struct silofs_vaddr *vaddr,
                             enum silofs_stg_mode stg_mode)
{
	...
	if (!silofs_vaddr_isnull(vaddr)) {
		err = check_stage_mode(exct, stg_mode);
	}
	return err;
}

static int
do_stage_vnode(struct silofs_exec_ctx *exct, const struct silofs_vaddr *vaddr,
               enum silofs_stg_mode stg_mode,
               struct silofs_vnode_info **out_vni)
{
	int err;

	err = check_stage_vnode(exct, vaddr, stg_mode);
	if (err) {
		return err;
	}
	err = do_resolve_stage_vnode(exct, vaddr, stg_mode, out_vni);
	if (err) {
		return err;
	}
	return 0;
...
static int
pre_stage_vnode(struct silofs_exec_ctx *exct, const struct silofs_vaddr *vaddr,
                enum silofs_stg_mode stg_mode)
{
	...
	if (vaddr->mtype != SILOFS_MTYPE_LSMAP) {
		...
		ret = silofs_stage_vnode(exct, nullptr, &vaddr2, stg_mode,
		                         &vni);
	}
	return ret;
}

int silofs_stage_vnode(struct silofs_exec_ctx *exct,
                       ...
                       struct silofs_vnode_info **out_vni)
{
	...
	err = pre_stage_vnode(exct, vaddr, stg_mode);
	if (!err) {
		err = do_stage_vnode(exct, vaddr, stg_mode, out_vni);
	}
...
static int
fetch_cached_ii(struct silofs_exec_ctx *exct, const struct silofs_vaddr *vaddr,
                struct silofs_inode_info **out_ii)
{
	struct silofs_vnode_info *vni = nullptr;
	int err;

	err = fetch_cached_vni(exct, vaddr, &vni);
	if (err) {
		return err;
	}
...
static int check_stage_inode(const struct silofs_exec_ctx *exct, ino_t ino,
                             enum silofs_stg_mode stg_mode)
{
	int err = -SILOFS_ENOENT;
...
static int resolve_stable_iaddr(struct silofs_exec_ctx *exct, ino_t ino,
                                struct silofs_vaddr *out_vaddr)
{
	...
	err = check_stable_at(exct, out_vaddr);
	if (err) {
		return err;
	}
	return 0;
...
static int stage_stable_inode_at(struct silofs_exec_ctx *exct,
                                 const struct silofs_vaddr *vaddr,
                                 enum silofs_stg_mode stg_mode,
                                 struct silofs_inode_info **out_ii)
{
	...
	err = stage_stable_vnode_at(exct, vaddr, stg_mode, &vni);
	if (err) {
		return err;
	}
	*out_ii = silofs_ii_from_vni(vni);
...
static int require_updated_cached_ii(struct silofs_exec_ctx *exct,
                                     struct silofs_inode_info *ii,
                                     enum silofs_stg_mode stg_mode)
{
	return require_updated_cached_vni(exct, &ii->i_vni, stg_mode);
}

static int do_resolve_stage_inode(struct silofs_exec_ctx *exct, ino_t ino,
                                  enum silofs_stg_mode stg_mode,
                                  struct silofs_inode_info **out_ii)
{
	struct silofs_vaddr vaddr;
	...
	err = resolve_stable_iaddr(exct, ino, &vaddr);
	if (err) {
		return err;
	}
	err = fetch_cached_ii(exct, &vaddr, out_ii);
	if (!err) {
		/* cache hit -- require up-to-date */
		err = require_updated_cached_ii(exct, *out_ii, stg_mode);
	} else {
		/* cache miss -- stage from objects store */
		err = stage_stable_inode_at(exct, &vaddr, stg_mode, out_ii);
	}
	return err;
}
...
int silofs_stage_inode(struct silofs_exec_ctx *exct, ino_t ino,
                       enum silofs_stg_mode stg_mode,
                       struct silofs_inode_info **out_ii)
{
	int err;

	err = check_stage_inode(exct, ino, stg_mode);
	if (err) {
		return err;
	}
	err = do_resolve_stage_inode(exct, ino, stg_mode, out_ii);
	if (err) {
		return err;
	}
...
int silofs_fetch_cached_inode(struct silofs_exec_ctx *exct, ino_t ino,
                              struct silofs_inode_info **out_ii)
{
...
static int
stage_raw_vnode(struct silofs_exec_ctx *exct, struct silofs_inode_info *pii,
                const struct silofs_vaddr *vaddr,
                struct silofs_vnode_info **out_vni)
{
	const enum silofs_stg_mode stg_mode = SILOFS_STG_COW | SILOFS_STG_RAW;

	return silofs_stage_vnode(exct, pii, vaddr, stg_mode, out_vni);
}

static int
spawn_vnode_at(struct silofs_exec_ctx *exct, struct silofs_inode_info *pii,
               const struct silofs_vaddr *vaddr,
               struct silofs_vnode_info **out_vni)
{
	int err;

	err = stage_raw_vnode(exct, pii, vaddr, out_vni);
	if (err) {
		return err;
	}
...
static int
do_spawn_vnode(struct silofs_exec_ctx *exct, struct silofs_inode_info *pii,
               enum silofs_mtype mtype, struct silofs_vnode_info **out_vni)
{
	...
	err = spawn_vnode_at(exct, pii, &vaddr, out_vni);
	if (err) {
		return err;
	}
	return 0;
...
int silofs_spawn_vnode(struct silofs_exec_ctx *exct,
                       ...
                       struct silofs_vnode_info **out_vni)
{
	...
	silofs_ii_incref(pii);
	err = do_spawn_vnode(exct, pii, mtype, out_vni);
	silofs_ii_decref(pii);
	return err;
}
...
static bool isock_allowed(const struct silofs_exec_ctx *exct)
{
	return (exct->env->flags & SILOFS_F_ALLOWISOCK) > 0;
}

static bool ififo_allowed(const struct silofs_exec_ctx *exct)
{
	return (exct->env->flags & SILOFS_F_ALLOWIFIFO) > 0;
}

static int check_itype(const struct silofs_exec_ctx *exct, mode_t mode)
{
	/*
	...
	case S_IFLNK:
		...
		break;
	case S_IFSOCK:
		ret = isock_allowed(exct) ? 0 : -SILOFS_EOPNOTSUPP;
		break;
	case S_IFIFO:
		ret = ififo_allowed(exct) ? 0 : -SILOFS_EOPNOTSUPP;
		break;
	case S_IFCHR:
	case S_IFBLK:
	default:
...
static int
claim_inode(struct silofs_exec_ctx *exct, struct silofs_inode_info **out_ii)
{
	struct silofs_vaddr vaddr;
	struct silofs_vnode_info *vni = nullptr;
	struct silofs_inode_info *ii  = nullptr;
	...
	err = silofs_claim_ispace(exct, &vaddr);
	if (err) {
		return err;
	}
	err = stage_raw_vnode(exct, nullptr, &vaddr, &vni);
	if (err) {
		return err;
	}
...
int silofs_spawn_inode(struct silofs_exec_ctx *exct,
                       const struct silofs_inew_params *inp,
                       struct silofs_inode_info **out_ii)
{
	int err;

	err = check_itype(exct, inp->mode);
	if (err) {
		return err;
	}
	err = claim_inode(exct, out_ii);
	if (err) {
		return err;
	}
...
static void forget_cached_vni(const struct silofs_exec_ctx *exct,
                              struct silofs_vnode_info *vni)
{
	silofs_lcache_forget_vni(exct->lcache, vni);
}

static int reclaim_vspace_at(struct silofs_exec_ctx *exct,
                             const struct silofs_vaddr *vaddr)
{
	struct silofs_llink llink;
...
static int
remove_vnode_of(struct silofs_exec_ctx *exct, struct silofs_vnode_info *vni)
{
	int err;

	silofs_vni_incref(vni);
	err = reclaim_vspace_at(exct, silofs_vni_vaddr(vni));
	silofs_vni_decref(vni);
	return err;
}

int silofs_remove_vnode(struct silofs_exec_ctx *exct,
                        struct silofs_vnode_info *vni)
{
	int err;

	err = remove_vnode_of(exct, vni);
	if (err) {
		return err;
	}
	forget_cached_vni(exct, vni);
	return 0;
}

int silofs_remove_vnode_at(struct silofs_exec_ctx *exct,
                           const struct silofs_vaddr *vaddr)
{
	struct silofs_vnode_info *vni = nullptr;
	int err;

	err = silofs_fetch_cached_vnode(exct, vaddr, &vni);
	if (!err) {
		err = silofs_remove_vnode(exct, vni);
	} else {
		err = reclaim_vspace_at(exct, vaddr);
	}
	return err;
}

static int
remove_inode_of(struct silofs_exec_ctx *exct, struct silofs_inode_info *ii)
{
	int err;

	silofs_ii_incref(ii);
	err = reclaim_vspace_at(exct, silofs_ii_vaddr(ii));
	silofs_ii_decref(ii);
	return err;
}

static void forget_cached_ii(const struct silofs_exec_ctx *exct,
                             struct silofs_inode_info *ii)
{
	silofs_assert_eq(ii->i_dq_vnis.dq.sz, 0);

	silofs_ii_undirtify(ii);
	forget_cached_vni(exct, &ii->i_vni);
}

int silofs_remove_inode(struct silofs_exec_ctx *exct,
                        struct silofs_inode_info *ii)
{
	int err;

	err = remove_inode_of(exct, ii);
	if (err) {
		return err;
	}
	forget_cached_ii(exct, ii);
	return 0;
}

/*: : : : : : : : : : : : : : : : : : : : : : : : : : : : : : : : : : : : : :*/

int silofs_refresh_llink(struct silofs_exec_ctx *exct,
                         struct silofs_vnode_info *vni)
{
	struct silofs_llink llink        = { .laddr.pos = -1 };
	const struct silofs_vaddr *vaddr = nullptr;
	...
	vaddr = silofs_vni_vaddr(vni);
	err   = silofs_resolve_llink_of(exct, vaddr, SILOFS_STG_CUR, &llink);
	if (err) {
