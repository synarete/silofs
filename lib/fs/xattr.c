                            ...
                            struct silofs_xanode_info **out_xai)
{
	...
	ii_incref(xa_ctx->ii);
	ret = silofs_stage_vnode(xa_ctx->sbi, vaddr, xa_ctx->stg_flags, &vi);
	if (ret) {
		goto out;
	}
	...
	ret = xac_recheck_node(xa_ctx, xai);
	if (ret) {
		goto out;
	}
	*out_xai = xai;
...
static int xac_getxattr(struct silofs_xattr_ctx *xa_ctx, size_t *out_size)
{
	...
	ii_incref(xa_ctx->ii);
	ret = xac_check_op(xa_ctx, R_OK);
	if (ret) {
		goto out;
	}
	ret = xac_lookup_entry(xa_ctx, &xei);
	if (ret) {
		goto out;
	}
...
static int xac_setxattr(struct silofs_xattr_ctx *xa_ctx)
{
	...
	ii_incref(xa_ctx->ii);
	ret = xac_check_op(xa_ctx, W_OK);
	if (ret) {
		goto out;
	}
	ret = xac_setxattr_new(xa_ctx);
	if (ret) {
		goto out;
	}
