                               ...
                               struct silofs_xanode_info **out_xai)
{
	...
	err = silofs_stage_vnode(xa_ctx->exct, xa_ctx->ii, vaddr,
	                         xa_ctx->stg_mode, &vni);
	if (err) {
		return err;
	}
...
{
	const struct silofs_env *env = xa_ctx->exct->env;
...
static int xac_check_op(const struct silofs_xattr_ctx *xa_ctx, int access_mode)
{
	...
	err = silofs_do_access(xa_ctx->exct, ii, access_mode);
	if (err) {
		return err;
	}
	return 0;
                            ...
                            struct silofs_xanode_info **out_xai)
{
	...
	err = silofs_spawn_vnode(xa_ctx->exct, xa_ctx->ii, SILOFS_MTYPE_XANODE,
	                         &vni);
	if (err) {
		return err;
	}
...
static int xac_remove_xanode_at(const struct silofs_xattr_ctx *xa_ctx,
                                const struct silofs_vaddr *vaddr)
{
	return silofs_remove_vnode_at(xa_ctx->exct, vaddr);
}
...
static void xac_update_post_setxattr(const struct silofs_xattr_ctx *xa_ctx)
{
	...
	silofs_update_iattrs_of(xa_ctx->exct, ii, &iattr);
}
...
static int xac_do_removexattr(struct silofs_xattr_ctx *xa_ctx)
{
	...
	xai_dirtify(xei.xai, xa_ctx->ii);
	silofs_update_itimes_of(xa_ctx->exct, xa_ctx->ii, SILOFS_IATTR_CTIME);
	return 0;
}
