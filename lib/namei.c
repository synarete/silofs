                                 ...
                                 const struct silofs_bootsecs *bsecs)
{
	...
	repos = repos_of(fs_ctx->fsc_uber);
                    ...
                    struct silofs_bootsecs *out_bsecs)
{
	...
	err = do_post_clone_updates(fs_ctx, out_bsecs);
	if (err) {
		return err;
	}
	return 0;
                   ...
                   struct silofs_bootsec *bsec_dst)
{
	...
	err = silofs_uber_pack_fs(uber, kivam, bsec_src, bsec_dst);
	if (err) {
		return err;
	}
	err = flush_dirty_now(uber);
                     ...
                     struct silofs_bootsec *bsec_dst)
{
	...
	err = silofs_uber_unpack_fs(uber, kivam, bsec_src, bsec_dst);
	if (err) {
		return err;
	}
	err = flush_dirty_now(uber);
