...
static void validate_ondisk_addrs(void)
{
	...
	REQUIRE_SIZEOF(struct silofs_pnptr192b, 192);
...
static void validate_ondisk_btnptr(void)
{
	REQUIRE_OFFSET64(struct silofs_btnptr256b, btp_base, 0);
...
{
	REQUIRE_OFFSET64(struct silofs_spdesc128b, spd_beg, 0);
	REQUIRE_OFFSET64(struct silofs_spdesc128b, spd_end, 64);
	REQUIRE_SIZEOF(struct silofs_spdesc128b, 128);
}
...
static void validate_ondisk_headers(void)
{
	...
	REQUIRE_OFFSETXX(struct silofs_header, h_stype, 14);
...
{
	REQUIRE_OFFSET64(struct silofs_uber_stat, ubs_btroot, 0);
	REQUIRE_OFFSET64(struct silofs_uber_stat, ubs_bn_spdesc, 256);
	REQUIRE_OFFSET64(struct silofs_uber_stat, ubs_vn_spdesc, 384);
	REQUIRE_OFFSET64(struct silofs_uber_stat, ubs_bn_count, 512);
	REQUIRE_OFFSET64(struct silofs_uber_stat, ubs_vn_count, 520);
	REQUIRE_SIZEOF(struct silofs_uber_stat, 1024);
	...
	REQUIRE_OFFSET64(struct silofs_uber_node, ub_stat, 1024);
...
static void validate_ondisk_space_node(void)
{
	...
	REQUIRE_OFFSET64(struct silofs_space_node, sp_ref_vtype, 24);
...
static void validate_ondisk_file(void)
{
	...
	REQUIRE_OFFSETXX(struct silofs_ftree_node, fn_child_vtype, 69);
