...
stc_read_pnode(struct silofs_store_ctx *st_ctx, struct silofs_pnode_info *pni)
{
	...
	return silofs_locos_read_blob(st_ctx->locos, &pni->pn_paddr, &rwvec);
}
...
{
	*out_bdi = silofs_create_cached_bldesc(st_ctx->pcache, paddr, spawn);
                            ...
                            struct silofs_bldesc_info **out_bdi)
{
	...
	err = stc_create_cached_bdi(st_ctx, paddr, true, out_bdi);
	if (err) {
		return err;
	}
                        ...
                        struct silofs_bldesc_info **out_bdi)
{
	...
	stc_init(&st_ctx, env);
	err = stc_spawn_bldesc(&st_ctx, paddr, out_bdi);
	stc_fini(&st_ctx);
	return err;
}

static int stc_lookup_cached_bdi(const struct silofs_store_ctx *st_ctx,
                                 const struct silofs_paddr *paddr,
                                 struct silofs_bldesc_info **out_bdi)
{
                            ...
                            struct silofs_bldesc_info **out_bdi)
{
	...
	err = stc_lookup_cached_bdi(st_ctx, paddr, &bdi);
	if (!err) {
		goto out_ok; /* OK -- cache hit */
	}
	err = stc_access_pnode(st_ctx, paddr);
	...
	err = stc_create_cached_bdi(st_ctx, paddr, false, &bdi);
	if (err) {
		return err;
	}
                        ...
                        struct silofs_bldesc_info **out_bdi)
{
	...
	stc_init(&st_ctx, env);
	err = stc_stage_bldesc(&st_ctx, paddr, out_bdi);
	stc_fini(&st_ctx);
	return err;
}
...
{
	*out_bti = silofs_create_cached_btnode(st_ctx->pcache, paddr, spawn);
                            ...
                            struct silofs_btnode_info **out_bti)
{
	...
	err = stc_create_cached_bti(st_ctx, paddr, true, out_bti);
	if (err) {
		return err;
	}
                        ...
                        struct silofs_btnode_info **out_bti)
{
	...
	stc_init(&st_ctx, env);
	err = stc_spawn_btnode(&st_ctx, paddr, out_bti);
	stc_fini(&st_ctx);
	return err;
}

static int stc_lookup_cached_bti(const struct silofs_store_ctx *st_ctx,
                                 const struct silofs_paddr *paddr,
                                 struct silofs_btnode_info **out_bti)
{
                            ...
                            struct silofs_btnode_info **out_bti)
{
	...
	err = stc_lookup_cached_bti(st_ctx, paddr, &bti);
	if (!err) {
		goto out_ok; /* OK -- cache hit */
	}
	err = stc_access_pnode(st_ctx, paddr);
	...
	err = stc_create_cached_bti(st_ctx, paddr, false, &bti);
	if (err) {
		return err;
	}
                        ...
                        struct silofs_btnode_info **out_bti)
{
	...
	stc_init(&st_ctx, env);
	err = stc_stage_btnode(&st_ctx, paddr, out_bti);
	stc_fini(&st_ctx);
	return err;
}
                           ...
                           const struct silofs_pnode_info *pni)
{
	...
	return silofs_locos_write_blob(st_ctx->locos, &pni->pn_paddr, &rovec);
}
