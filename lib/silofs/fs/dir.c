...
{
	STATICASSERT_LT(SILOFS_DTREE_INDEX_MAX, INT32_MAX);
	STATICASSERT_LT(SILOFS_DTREE_DEPTH_MAX,
	                SILOFS_HASH256_LEN / SILOFS_DTREE_NODE_SHIFT);
...
static void laddr_of_dnode(struct silofs_laddr *laddr, off_t off)
{
	silofs_laddr_setup(laddr, SILOFS_LTYPE_DTNODE, off);
}
...
{
	return (dtn_index == SILOFS_DTREE_INDEX_NULL);
}

static bool dtn_index_isroot(silofs_dtn_index_t dtn_index)
{
	return (dtn_index == SILOFS_DTREE_INDEX_ROOT);
}
...
static bool dtn_index_isvalid(silofs_dtn_index_t dtn_index)
{
	...
	return (dtn_index <= SILOFS_DTREE_INDEX_MAX);
}

static silofs_dtn_index_t dtn_index_to_parent(silofs_dtn_index_t dtn_index)
{
	silofs_dtn_index_t parent_index = SILOFS_DTREE_INDEX_NULL;

	if (!dtn_index_isroot(dtn_index)) {
		parent_index = (dtn_index - 1) / SILOFS_DTREE_NODE_NCHILDS;
	}
	return parent_index;
}
...
{
	return (dtn_index - 1) % SILOFS_DTREE_NODE_NCHILDS;
}
...
{
	return parent_dtn_index * SILOFS_DTREE_NODE_NCHILDS + child_ord + 1;
}
...
static silofs_dtn_depth_t dtn_index_depth(silofs_dtn_index_t dtn_index)
{
	...
	while (dtn_index > SILOFS_DTREE_INDEX_ROOT) {
		if (++depth > SILOFS_DTREE_DEPTH_MAX) {
...
{
	return (depth <= SILOFS_DTREE_DEPTH_MAX);
}
...
hash_to_child_ord(uint64_t hash, silofs_dtn_depth_t depth)
{
	...
	silofs_expect_le(depth, SILOFS_DTREE_DEPTH_MAX);
...
static silofs_dtn_index_t
hash_to_child_dtn_index(uint64_t hash, silofs_dtn_index_t parent_dtn_idx)
{
	const silofs_dtn_depth_t depth   = dtn_index_depth(parent_dtn_idx);
	const silofs_dtn_ord_t child_ord = hash_to_child_ord(hash, depth + 1);

	return child_dtn_index_of(parent_dtn_idx, child_ord);
}
...
{
	const uint64_t mask = (UINT64_C(1) << SILOFS_DTREE_OFF_SHIFT) - 1;

	*out_doff = (dtn_index << SILOFS_DTREE_OFF_SHIFT) | (slot & mask);
}
...
decode_doffset(uint64_t doff, uint64_t *out_dtn_index, uint64_t *out_slot)
{
	...
	*out_dtn_index = (doff >> SILOFS_DTREE_OFF_SHIFT);
	*out_slot      = (doff & mask);
}
...
static off_t make_doffset(size_t dtn_index, size_t slot)
{
	...
	STATICASSERT_EQ(SILOFS_DTREE_NODE_SIZE, 1 << SILOFS_DTREE_OFF_SHIFT);
...
static silofs_dtn_index_t doff_to_dtn_index(off_t doff)
{
	...
	decode_doffset((uint64_t)(doff >> 2), &dtn_idx, &slot);
	if (dtn_idx > SILOFS_DTREE_INDEX_MAX) {
		dtn_idx = SILOFS_DTREE_INDEX_NULL;
	}
	return dtn_idx;
}
...
static bool de_isvalid(const struct silofs_dir_entry *de, size_t names_pos_beg)
{
...
static void
dtn_set_node_index(struct silofs_dtree_node *dtn, silofs_dtn_index_t index)
{
...
dtn_child_off(const struct silofs_dtree_node *dtn, silofs_dtn_ord_t ord)
{
	...
	silofs_laddr56_xtoh(&dtn->dn_child[ord], &off);
	return off;
}

static void dtn_child(const struct silofs_dtree_node *dtn,
                      silofs_dtn_ord_t ord, struct silofs_laddr *out_laddr)
{
	const off_t off = dtn_child_off(dtn, ord);

	silofs_laddr_setup(out_laddr, SILOFS_LTYPE_DTNODE, off);
}

static bool
dtn_has_child_at(const struct silofs_dtree_node *dtn, silofs_dtn_ord_t ord)
{
	struct silofs_laddr laddr;

	dtn_child(dtn, ord, &laddr);
	return !laddr_isnull(&laddr);
}

static void dtn_set_child(struct silofs_dtree_node *dtn, silofs_dtn_ord_t ord,
                          const struct silofs_laddr *laddr)
{
	silofs_laddr56_htox(&dtn->dn_child[ord], laddr->off);
}
...
static void dtn_reset_childs(struct silofs_dtree_node *dtn)
{
	...
	for (ord = 0; ord < ARRAY_SIZE(dtn->dn_child); ++ord) {
		dtn_set_child(dtn, ord, silofs_laddr_none());
	}
}

static void dtn_setup(struct silofs_dtree_node *dtn, ino_t ino,
                      silofs_dtn_index_t dtn_idx, off_t parent_off)
{
	silofs_assert_le(dtn_idx, SILOFS_DTREE_INDEX_MAX);

	dtn_set_ino(dtn, ino);
	dtn_set_parent(dtn, parent_off);
	dtn_set_node_index(dtn, dtn_idx);
	dtn_set_nde(dtn, 0);
	dtn_set_nnb(dtn, 0);
...
{
	const silofs_dtn_index_t dtn_idx = dtn_node_index(dtn);

	return dtn_index_depth(dtn_idx);
}
...
static struct silofs_dir_entry *dtn_de_end(const struct silofs_dtree_node *dtn)
{
...
static size_t dtn_names_beg_pos(const struct silofs_dtree_node *dtn)
{
	const size_t nnb = dtn_nnb(dtn);

	silofs_assert_lt(nnb, ARRAY_SIZE(dtn->dn_data.nb));
	return ARRAY_SIZE(dtn->dn_data.nb) - nnb;
}
...
static const struct silofs_dir_entry *
dtn_search_nonactive(const struct silofs_dtree_node *dtn)
{
	const struct silofs_dir_entry *de_beg = dtn_de_begin(dtn);
	const struct silofs_dir_entry *de_end = dtn_de_end(dtn);
	const struct silofs_dir_entry *de;
...
static bool dtn_may_insert(const struct silofs_dtree_node *dtn, size_t nlen)
{
	...
	nused = dtn_data_used(dtn);
	silofs_assert_le(nused, limit);

	/* trivial case: has enough room */
	if ((nwant + nused) <= limit) {
		return true;
	}
	/* edge case: there exists a free de, just enough to use this node */
	de = dtn_search_nonactive(dtn);
	if ((de != nullptr) && ((nwant + nused - sizeof(*de)) <= limit)) {
		return true;
	}
	/* not enough space in this node */
	return false;
}
...
static bool dtn_punch_name_at(struct silofs_dtree_node *dtn, size_t name_pos,
                              size_t name_len)
{
	...
	if (name > names_beg) {
		const size_t mv_len = (size_t)(name - names_beg);
...
static bool
dtn_remove_de_name(struct silofs_dtree_node *dtn, struct silofs_dir_entry *de)
{
	const size_t name_len = de_name_len(de);
	bool punched;
                             ...
                             size_t name_pos_ref, size_t name_len)
{
	...
	for (struct silofs_dir_entry *de = de_beg; de < de_end; ++de) {
		size_t name_pos;
...
static void
dtn_remove(struct silofs_dtree_node *dtn, struct silofs_dir_entry *de)
{
	const size_t name_len = de_name_len(de);
	const size_t name_pos = de_name_pos(de);
	bool punched;
...
static void
dtn_child_by_ord(const struct silofs_dtree_node *dtn, silofs_dtn_ord_t ord,
                 struct silofs_laddr *out_laddr)
{
	dtn_child(dtn, ord, out_laddr);
}

static void dtn_child_by_hash(const struct silofs_dtree_node *dtn,
                              uint64_t hash, struct silofs_laddr *out_laddr)
{
	silofs_dtn_ord_t ord;
	const silofs_dtn_depth_t depth = dtn_depth(dtn);

	ord = hash_to_child_ord(hash, depth + 1);
	dtn_child_by_ord(dtn, ord, out_laddr);
}

static void dtn_parent_addr(const struct silofs_dtree_node *dtn,
                            struct silofs_laddr *out_laddr)
{
	laddr_of_dnode(out_laddr, dtn_parent(dtn));
}
...
static int dtn_verify_des(const struct silofs_dtree_node *dtn)
{
	const struct silofs_dir_entry *de_beg = dtn_de_begin(dtn);
	const struct silofs_dir_entry *de_end = dtn_de_end(dtn);
	size_t names_beg_pos;

	names_beg_pos = dtn_names_beg_pos(dtn);
	for (const struct silofs_dir_entry *de = de_beg; de < de_end; ++de) {
		if (!de_isactive(de)) {
			continue;
		}
		if (!de_isvalid(de, names_beg_pos)) {
...
static int dtn_verify_counts(const struct silofs_dtree_node *dtn)
{
	if (dtn_nnb(dtn) > sizeof(dtn->dn_data.nb)) {
		return -SILOFS_EFSCORRUPTED;
	}
	if (dtn_nde(dtn) > ARRAY_SIZE(dtn->dn_data.de)) {
		return -SILOFS_EFSCORRUPTED;
	}
	if (dtn_data_used(dtn) > sizeof(dtn->dn_data)) {
		return -SILOFS_EFSCORRUPTED;
	}
	return 0;
}
...
dti_unconst(const struct silofs_dtnode_info *dti)
{
	return silofs_unconst(dti);
}

static void dti_incref(struct silofs_dtnode_info *dti)
{
	if (likely(dti != nullptr)) {
		silofs_lni_incref(&dti->dtn_lni);
	}
}

static void dti_decref(struct silofs_dtnode_info *dti)
{
	if (likely(dti != nullptr)) {
		silofs_lni_decref(&dti->dtn_lni);
	}
}

static const struct silofs_laddr *
dti_laddr(const struct silofs_dtnode_info *dti)
{
	return silofs_lni_laddr(&dti->dtn_lni);
}

static void dti_get_laddr(const struct silofs_dtnode_info *dti,
                          struct silofs_laddr *out_laddr)
{
	silofs_laddr_assign(out_laddr, dti_laddr(dti));
}
...
dti_child_addr_by_hash(const struct silofs_dtnode_info *dti, uint64_t hash,
                       struct silofs_laddr *out_laddr)
{
	dtn_child_by_hash(dti->dtn, hash, out_laddr);
}

static void
dti_child_addr_by_ord(const struct silofs_dtnode_info *dti,
                      silofs_dtn_ord_t ord, struct silofs_laddr *out_laddr)
{
	dtn_child_by_ord(dti->dtn, ord, out_laddr);
}

static void
dti_setup_dnode(struct silofs_dtnode_info *dti, ino_t ino,
                const struct silofs_laddr *parent, silofs_dtn_index_t dtn_idx)
{
	dtn_setup(dti->dtn, ino, dtn_idx, parent->off);
}

static int dti_check_child_depth(const struct silofs_dtnode_info *dti,
                                 silofs_dtn_depth_t parent_depth)
{
	const struct silofs_laddr *laddr     = dti_laddr(dti);
	const silofs_dtn_depth_t child_depth = dtn_depth(dti->dtn);
	int err                              = 0;

	if ((parent_depth + 1) != child_depth) {
		log_err("illegal-tree-depth: voff=0x%lx "
		        "parent_depth=%u child_depth=%u ",
		        laddr->off, parent_depth, child_depth);
...
static void
dti_bind_child_at(struct silofs_dtnode_info *parent_dti,
                  const struct silofs_laddr *laddr, silofs_dtn_index_t dtn_idx)
{
	struct silofs_dtree_node *dtn    = parent_dti->dtn;
	const silofs_dtn_ord_t child_ord = dtn_index_to_child_ord(dtn_idx);
	const bool rebind                = dtn_has_child_at(dtn, child_ord);

	dtn_set_child(dtn, child_ord, laddr);
	if (!rebind) {
...
static uint64_t dirin_seed(const struct silofs_inode_dir *dirin)
{
	return silofs_le64_to_cpu(dirin->d_seed);
}

static void dirin_set_seed(struct silofs_inode_dir *dirin, size_t seed)
{
	dirin->d_seed = silofs_cpu_to_le64(seed);
}

static uint64_t dirin_ndents(const struct silofs_inode_dir *dirin)
{
	return silofs_le64_to_cpu(dirin->d_ndents);
}

static void dirin_set_ndents(struct silofs_inode_dir *dirin, size_t n)
{
	dirin->d_ndents = silofs_cpu_to_le64(n);
}
...
static silofs_dtn_index_t
dirin_last_index(const struct silofs_inode_dir *dirin)
{
	return silofs_le32_to_cpu(dirin->d_last_index);
}

static void dirin_set_last_index(struct silofs_inode_dir *dirin, size_t index)
{
	dirin->d_last_index = silofs_cpu_to_le32((uint32_t)index);
}

static void dirin_update_last_index(struct silofs_inode_dir *dirin,
                                    silofs_dtn_index_t alt_index, bool add)
{
	const silofs_dtn_index_t cur_idx = dirin_last_index(dirin);
	const silofs_dtn_index_t nil_idx = SILOFS_DTREE_INDEX_NULL;
	silofs_dtn_index_t new_idx       = cur_idx;

	if (add) {
		if ((cur_idx < alt_index) || dtn_index_isnull(cur_idx)) {
			new_idx = alt_index;
		}
	} else {
		if (cur_idx == alt_index) {
			new_idx = !cur_idx ? nil_idx : cur_idx - 1;
		}
	}
	dirin_set_last_index(dirin, new_idx);
}

static void dirin_tree_root(const struct silofs_inode_dir *dirin,
                            struct silofs_laddr *out_laddr)
{
	silofs_laddr64_xtoh(&dirin->d_root, out_laddr);
}

static void dirin_set_tree_root(struct silofs_inode_dir *dirin,
                                const struct silofs_laddr *laddr)
{
	silofs_laddr64_htox(&dirin->d_root, laddr);
}

static enum silofs_dirf dirin_flags(const struct silofs_inode_dir *dirin)
{
	const uint32_t flags = silofs_le32_to_cpu(dirin->d_flags);

	return (enum silofs_dirf)flags;
}

static void
dirin_set_flags(struct silofs_inode_dir *dirin, enum silofs_dirf flags)
{
	dirin->d_flags = silofs_cpu_to_le32((uint32_t)flags);
}

static enum silofs_namehfn dirin_hashfn(const struct silofs_inode_dir *dirin)
{
	return (enum silofs_namehfn)(dirin->d_hashfn);
}

static void
dirin_set_hashfn(struct silofs_inode_dir *dirin, enum silofs_namehfn hfn)
{
...
{
	dirin_set_tree_root(dirin, silofs_laddr_none());
	dirin_set_last_index(dirin, SILOFS_DTREE_INDEX_NULL);
...
static void dir_tree_root(const struct silofs_inode_info *dir_ii,
                          struct silofs_laddr *out_laddr)
{
	dirin_tree_root(dir_ispec_of(dir_ii), out_laddr);
}
                              ...
                              const struct silofs_laddr *laddr)
{
	...
	dirin_set_tree_root(dirin, laddr);
	dirin_set_last_index(dirin, SILOFS_DTREE_INDEX_ROOT);
}

static silofs_dtn_index_t
dir_last_index(const struct silofs_inode_info *dir_ii)
{
	return dirin_last_index(dir_ispec_of(dir_ii));
}

static void dir_update_last_index(struct silofs_inode_info *dir_ii,
                                  silofs_dtn_index_t alt_idx, bool add)
{
	dirin_update_last_index(dir_ispec_of(dir_ii), alt_idx, add);
}

static uint64_t dir_seed(const struct silofs_inode_info *dir_ii)
{
	return dirin_seed(dir_ispec_of(dir_ii));
}

static enum silofs_dirf dir_flags(const struct silofs_inode_info *dir_ii)
{
	return dirin_flags(dir_ispec_of(dir_ii));
}
...
{
	return dirin_hashfn(dir_ispec_of(dir_ii));
}
                          ...
                          struct silofs_namestr *out_nstr)
{
	...
	enum silofs_namehfn nhfn = dir_hfn(dir_ii);
	int err;
...
static int search_dnode(const struct silofs_dtnode_info *dti,
                        const struct silofs_namestr *name,
                        struct silofs_dir_entry_info *out_dei)
{
	const struct silofs_dir_entry *de;

	de = dtn_search(dti->dtn, name);
	if (de == nullptr) {
...
static int dirc_stage_dtnode(const struct silofs_dir_ctx *d_ctx,
                             const struct silofs_laddr *laddr,
...
static int dirc_stage_child(const struct silofs_dir_ctx *d_ctx,
                            struct silofs_dtnode_info *parent_dti,
                            const struct silofs_laddr *laddr,
                            struct silofs_dtnode_info **out_dti)
{
	int ret;
	...
	ret = dirc_stage_dtnode(d_ctx, laddr, out_dti);
	dti_decref(parent_dti);
	return ret;
}

static int dirc_stage_child_by_name(const struct silofs_dir_ctx *d_ctx,
                                    struct silofs_dtnode_info *parent_dti,
                                    struct silofs_dtnode_info **out_dti)
{
	struct silofs_laddr laddr;
	...
	dti_child_addr_by_hash(parent_dti, d_ctx->name->hash, &laddr);
	if (!laddr_isnull(&laddr)) {
		err = dirc_stage_child(d_ctx, parent_dti, &laddr, out_dti);
	}
	return err;
}
...
{
	silofs_update_isize(d_ctx->task, d_ctx->dir_ii, sz);
}

static void dirc_update_iblocks(const struct silofs_dir_ctx *d_ctx, int dif)
{
	silofs_update_iblocks(d_ctx->task, d_ctx->dir_ii, SILOFS_LTYPE_DTNODE,
	                      dif);
}
                                    ...
                                    silofs_dtn_index_t dtn_index, bool newn)
{
	...
	silofs_dtn_index_t last_dtn_idx  = 0;

	dir_update_last_index(dir_ii, dtn_index, newn);
	last_dtn_idx = dir_last_index(dir_ii);

	dirc_update_isize(d_ctx, dtn_index_to_isize(last_dtn_idx));
	dirc_update_iblocks(d_ctx, newn ? 1 : -1);
}

static int dirc_spawn_setup_dnode(const struct silofs_dir_ctx *d_ctx,
                                  const struct silofs_laddr *parent,
...
static void dirc_resolve_tree(const struct silofs_dir_ctx *d_ctx,
                              struct silofs_laddr *out_laddr)
{
	dir_tree_root(d_ctx->dir_ii, out_laddr);
}

static int dirc_stage_tree_root(const struct silofs_dir_ctx *d_ctx,
                                struct silofs_dtnode_info **out_dti)
{
	struct silofs_laddr laddr = { .off = SILOFS_OFF_NULL };
	int ret                   = -SILOFS_ENOENT;

	dirc_resolve_tree(d_ctx, &laddr);
	if (!laddr_isnull(&laddr)) {
		ret = dirc_stage_dtnode(d_ctx, &laddr, out_dti);
	}
	return ret;
                                ...
                                struct silofs_dtnode_info **out_dti)
{
	...
	const silofs_dtn_index_t dtn_idx = SILOFS_DTREE_INDEX_ROOT;
	int err;

	laddr_of_dnode(&laddr, SILOFS_OFF_NULL);
	err = dirc_spawn_setup_dnode(d_ctx, &laddr, dtn_idx, out_dti);
	return_if_err(err);

	dir_set_tree_root(d_ctx->dir_ii, dti_laddr(*out_dti));
	dir_ii_setdirty(d_ctx->dir_ii);
	return 0;
}
                                  ...
                                  struct silofs_dtnode_info **out_dti)
{
	...
	if (dirc_has_tree(d_ctx)) {
		err = dirc_stage_tree_root(d_ctx, out_dti);
	} else {
		err = dirc_spawn_tree_root(d_ctx, out_dti);
	}
	return err;
}
...
static int dirc_do_lookup_by_tree(const struct silofs_dir_ctx *d_ctx,
                                  struct silofs_dtnode_info *root_dti,
                                  struct silofs_dir_entry_info *dei)
{
	const struct silofs_namestr *name    = d_ctx->name;
	struct silofs_dtnode_info *child_dti = nullptr;
	struct silofs_dtnode_info *dti       = root_dti;
	silofs_dtn_depth_t depth;
	int ret = -SILOFS_ENOENT;

	for (depth = dtn_depth(dti->dtn); depth_isvalid(depth); ++depth) {
		ret = search_dnode(dti, name, dei);
		if (!ret || (ret != -SILOFS_ENOENT)) {
			break;
		}
		ret = dirc_stage_child_by_name(d_ctx, dti, &child_dti);
		if (ret) {
			break;
		}
		if (child_dti == nullptr) { /* make gcc-analyzer happy */
			ret = -SILOFS_ENOENT;
			break;
		}
		dti = child_dti;
		ret = dti_check_child_depth(dti, depth);
		if (ret) {
			break;
		}
		ret = -SILOFS_ENOENT;
                               ...
                               struct silofs_dir_entry_info *dei)
{
	...
	dti_incref(root_dti);
	ret = dirc_do_lookup_by_tree(d_ctx, root_dti, dei);
	dti_decref(root_dti);
	return ret;
}
                               ...
                               struct silofs_dir_entry_info *dei)
{
	...
	err = dirc_lookup_by_tree(d_ctx, dti, dei);
	return_if_err(err);
...
static int dirc_check_self_and_name(const struct silofs_dir_ctx *d_ctx)
{
	...
	if (!silofs_ii_isdir(d_ctx->dir_ii)) {
...
static int dirc_check_and_lookup_by_name(const struct silofs_dir_ctx *d_ctx,
                                         struct silofs_dir_entry_info *dei)
{
	int err;
                              ...
                              struct silofs_ino_dt *out_idt)
{
	...
	silofs_ii_incref(d_ctx->dir_ii);
	ret = dirc_do_lookup_dentry(d_ctx, out_idt);
	silofs_ii_decref(d_ctx->dir_ii);
	return ret;
}
...
static void dirc_add_dti_to_predq(const struct silofs_dir_ctx *d_ctx,
                                  struct silofs_dtnode_info *dti)
{
	const struct silofs_core_refs *corefs = d_ctx->task->corefs;
                 ...
                 struct silofs_dtnode_info **out_dti)
{
	...
	dirc_add_dti_to_predq(d_ctx, *out_dti);
	return 0;
}
...
{
	const struct silofs_namestr *name       = d_ctx->name;
	const struct silofs_laddr *parent_laddr = dti_laddr(parent_dti);
	silofs_dtn_index_t parent_dtn_idx, child_dtn_idx;
	int err;

	parent_dtn_idx = dtn_node_index(parent_dti->dtn);
	child_dtn_idx  = hash_to_child_dtn_index(name->hash, parent_dtn_idx);

	err = dirc_spawn_child(d_ctx, parent_laddr, child_dtn_idx, out_dti);
	return_if_err(err);

	dti_bind_child_at(parent_dti, dti_laddr(*out_dti), child_dtn_idx);

	dirc_add_dti_to_predq(d_ctx, parent_dti);
	return 0;
}
                                 ...
                                 struct silofs_dtnode_info **out_dti)
{
	...
	dti_incref(parent_dti);
	ret = dirc_do_spawn_bind_child(d_ctx, parent_dti, out_dti);
	dti_decref(parent_dti);
	return ret;
}
...
static void dirc_resolve_child_of(const struct silofs_dir_ctx *d_ctx,
                                  ...
                                  struct silofs_laddr *out_laddr)
{
	dti_child_addr_by_hash(dti, d_ctx->name->hash, out_laddr);
}
                              ...
                              struct silofs_dtnode_info **out_dti)
{
	...
	dirc_resolve_child_of(d_ctx, parent_dti, &laddr);
	if (!laddr_isnull(&laddr)) {
		err = dirc_stage_child(d_ctx, parent_dti, &laddr, out_dti);
	} else {
...
static int dirc_discard_dnode(const struct silofs_dir_ctx *d_ctx,
                              struct silofs_dtnode_info *dti)
{
	const silofs_dtn_index_t dtn_index = dtn_node_index(dti->dtn);
	int err;

	dtn_reset_data(dti->dtn);
	err = dirc_remove_dtnode(d_ctx, dti);
	return_if_err(err);
...
{
	return (nlink_t)((long)silofs_ii_nlink(ii) + dif);
}
...
static void dirc_update_nlink(const struct silofs_dir_ctx *d_ctx, long dif)
{
	...
	silofs_update_iattrs(d_ctx->task, child_ii, &iattr);
	...
	silofs_update_iattrs(d_ctx->task, dir_ii, &iattr);
}
                             ...
                             struct silofs_dtnode_info *dti)
{
	...
	if (!dtn_may_insert(dti->dtn, d_ctx->name->sv.len)) {
		return -SILOFS_ENOSPC;
	}
	dtn_insert(dti->dtn, d_ctx->name, ii->i_ino, ii_dtype_of(ii));
	dir_inc_ndents(d_ctx->dir_ii);

	dirc_add_dti_to_predq(d_ctx, dti);
	return 0;
}
                               ...
                               struct silofs_dtnode_info *root_dti)
{
	...
	for (depth = dtn_depth(dti->dtn); depth_isvalid(depth); ++depth) {
		ret = dirc_add_to_dnode(d_ctx, dti);
		if (ret == 0) {
			break;
		}
		ret = dirc_require_child(d_ctx, dti, &dti);
		if (ret) {
			break;
		}
		ret = dti_check_child_depth(dti, depth);
                            ...
                            struct silofs_dtnode_info *root_dti)
{
	...
	dti_incref(root_dti);
	ret = dirc_do_add_to_tree(d_ctx, root_dti);
	dti_decref(root_dti);
	return ret;
}
                              ...
                              struct silofs_dtnode_info *root_dti)
{
	...
	err = dirc_add_to_tree(d_ctx, root_dti);
	return_if_err(err);
...
{
	struct silofs_dtnode_info *root_dti = nullptr;
	int err;

	err = dirc_require_tree_root(d_ctx, &root_dti);
	return_if_err(err);

	err = dirc_insert_dentry(d_ctx, root_dti);
	return_if_err(err);
...
static int dirc_add_dentry(struct silofs_dir_ctx *d_ctx)
{
	...
	silofs_ii_incref(d_ctx->dir_ii);
	silofs_ii_incref(d_ctx->child_ii);
	ret = dirc_do_add_dentry(d_ctx);
	silofs_ii_decref(d_ctx->child_ii);
	silofs_ii_decref(d_ctx->dir_ii);
	return ret;
}
...
{
	return silofs_stage_inode_by(d_ctx->task, ino, d_ctx->stg_mode,
	                             out_ii);
}
...
static int dirc_check_stage_parent(struct silofs_dir_ctx *d_ctx)
{
	...
	parent = silofs_ii_parent(d_ctx->dir_ii);
...
dirc_isindex_inrange(const struct silofs_dir_ctx *d_ctx, size_t dtn_index)
{
	...
	const size_t dtn_index_null = SILOFS_DTREE_INDEX_NULL;
	bool ret;
                             ...
                             const struct silofs_inode_info *ii)
{
	...
	const struct stat *attr = nullptr;
                             ...
                             const struct silofs_dtnode_info *dti)
{
	...
	while (!dirc_stopped(d_ctx)) {
		de = dtn_scan(dti->dtn, d_ctx->rd_ctx->pos);
		if (!de) {
			off                = dtn_next_doffset(dti->dtn);
			d_ctx->rd_ctx->pos = off;
			break;
		}
		off = dtn_doffset_of(dti->dtn, de);
		err = dirc_stage_inode_of_de(d_ctx, de, &ii);
		if (err) {
			return err;
		}
		ok = dirc_emit_dirent(d_ctx, dti->dtn, de, off, ii);
		if (!ok) {
			break;
		}
		d_ctx->rd_ctx->pos += 1;
...
{
	d_ctx->rd_ctx->pos = -1;
	dirc_emit(d_ctx, "", 0, SILOFS_INO_NULL, 0, nullptr);
                                      ...
                                      struct silofs_dtnode_info **out_dti)
{
	...
	if (ord >= SILOFS_DTREE_NODE_NCHILDS) {
		goto out;
	}
	dti_child_addr_by_ord(dti, ord, &laddr);
	if (laddr_isnull(&laddr)) {
		goto out;
	}
	ret = dirc_stage_dtnode(d_ctx, &laddr, out_dti);
...
static int
dirc_stage_child_by_ord(const struct silofs_dir_ctx *d_ctx,
                        struct silofs_dtnode_info *dti, silofs_dtn_ord_t ord,
                        struct silofs_dtnode_info **out_dti)
{
	int ret;

	dti_incref(dti);
	ret = dirc_do_stage_child_by_ord(d_ctx, dti, ord, out_dti);
	dti_decref(dti);
	return ret;
}

static int dirc_stage_node_by_index(const struct silofs_dir_ctx *d_ctx,
                                    const struct silofs_dtnode_info *root_dti,
                                    silofs_dtn_index_t dtn_index,
                                    struct silofs_dtnode_info **out_dti)
{
	silofs_dtn_ord_t child_ord[SILOFS_DTREE_DEPTH_MAX];
	silofs_dtn_depth_t depth;
	silofs_dtn_ord_t ord;
	int err;
	...
	*out_dti = dti_unconst(root_dti);
	for (size_t i = 0; i < depth; ++i) {
		ord = child_ord[i];
		err = dirc_stage_child_by_ord(d_ctx, *out_dti, ord, out_dti);
		if (err) {
			return err;
		}
	}
...
static int dirc_next_node(struct silofs_dir_ctx *d_ctx,
                          const struct silofs_dtnode_info *dti,
                          silofs_dtn_index_t *out_dtn_idx)
{
	struct silofs_laddr laddr = {
		.off = -1,
	};
	struct silofs_dtnode_info *parent_dti = nullptr;
	silofs_dtn_index_t next_dtn_idx, curr_dtn_idx;
	silofs_dtn_ord_t ord;
	int err;

	curr_dtn_idx = dtn_node_index(dti->dtn);
	next_dtn_idx = curr_dtn_idx + 1;

	dtn_parent_addr(dti->dtn, &laddr);
	if (laddr_isnull(&laddr)) {
		*out_dtn_idx = next_dtn_idx;
		return 0;
	}

	err = dirc_stage_dtnode(d_ctx, &laddr, &parent_dti);
	return_if_err(err);

	ord = dtn_child_ord(dti->dtn);
	while (++ord < SILOFS_DTREE_NODE_NCHILDS) {
		dti_child_addr_by_ord(parent_dti, ord, &laddr);
		if (!laddr_isnull(&laddr)) {
			break;
		}
		next_dtn_idx += 1;
...
static int
dirc_do_iterate_tree_nodes(struct silofs_dir_ctx *d_ctx,
                           const struct silofs_dtnode_info *root_dti)
{
	silofs_dtn_index_t dtn_idx;
	int ret = 0;

	dtn_idx = dirc_curr_node_index_of(d_ctx);
	if (dtn_index_isnull(dtn_idx)) {
		dtn_idx = SILOFS_DTREE_INDEX_ROOT;
	}
	while (!ret && !dirc_stopped(d_ctx)) {
		struct silofs_dtnode_info *dti = nullptr;

		if (!dirc_isindex_inrange(d_ctx, dtn_idx)) {
			ret = dirc_readdir_eos(d_ctx);
			break;
		}
		ret = dirc_stage_node_by_index(d_ctx, root_dti, dtn_idx, &dti);
		...
		ret = dirc_iterate_node(d_ctx, dti);
		if (ret) {
			break;
		}
		ret = dirc_next_node(d_ctx, dti, &dtn_idx);
	}
	return ret;
}
                                   ...
                                   struct silofs_dtnode_info *root_dti)
{
	...
	dti_incref(root_dti);
	ret = dirc_do_iterate_tree_nodes(d_ctx, root_dti);
	dti_decref(root_dti);
	return ret;
}
...
{
	struct silofs_dtnode_info *dti = nullptr;
	silofs_dtn_index_t start_idx;
	int err;

	start_idx = dirc_curr_node_index_of(d_ctx);
	if (!dirc_isindex_inrange(d_ctx, start_idx)) {
	...
	err = dirc_iterate_tree_nodes(d_ctx, dti);
	if (err) {
		return err;
	}
	return 0;
...
static bool dirc_emit_meta(struct silofs_dir_ctx *d_ctx)
{
	if (d_ctx->rd_ctx->pos == 0) {
		if (!dirc_emit_dot(d_ctx)) {
			return false;
		}
		d_ctx->rd_ctx->pos = 1;
	}
	if (d_ctx->rd_ctx->pos == 1) {
		if (!dirc_emit_dotdot(d_ctx)) {
			return false;
		}
		d_ctx->rd_ctx->pos = 2;
	}
...
static void dirc_post_readdir(const struct silofs_dir_ctx *d_ctx)
{
	...
	silofs_update_iattrs(d_ctx->task, d_ctx->dir_ii, &iattr);
}
...
static int dirc_discard_childs_of(const struct silofs_dir_ctx *d_ctx,
                                  struct silofs_dtnode_info *dti)
{
	struct silofs_laddr child_laddr;
	silofs_dtn_ord_t ord;
	int err = 0;

	dti_incref(dti);
	for (ord = 0; ord < SILOFS_DTREE_NODE_NCHILDS; ++ord) {
		if (!dtn_nactive_childs(dti->dtn)) {
			break;
		}
		dtn_child_by_ord(dti->dtn, ord, &child_laddr);
		if (laddr_isnull(&child_laddr)) {
			continue;
		}
		err = dirc_discard_recursively(d_ctx, &child_laddr);
		if (err) {
			break;
		}
		dtn_dec_nactive_childs(dti->dtn);
	}
	dti_decref(dti);
	return err;
}
                                ...
                                struct silofs_dtnode_info *dti)
{
	...
	err = dirc_discard_childs_of(d_ctx, dti);
	return_if_err(err);

	err = dirc_discard_dnode(d_ctx, dti);
	return_if_err(err);
                                    ...
                                    const struct silofs_laddr *laddr)
{
	...
	err = dirc_stage_dtnode(d_ctx, laddr, &dti);
	return_if_err(err);
...
{
	dtn_remove(dei->dti->dtn, dei->de);
	dir_dec_ndents(d_ctx->dir_ii);
	dirc_update_nlink(d_ctx, -1);
	if (!dir_ndents(d_ctx->dir_ii)) {
		return dirc_erase_empty_tree(d_ctx);
	}
	dirc_add_dti_to_predq(d_ctx, dei->dti);
	return 0;
}
...
{
	const struct silofs_inode_dir *dirin = dirin_of(inode);
	struct silofs_laddr laddr            = { .off = -1 };
	silofs_dtn_index_t dtn_idx;
	int err;

	dirin_tree_root(dirin, &laddr);
	if (!silofs_off_isnull(laddr.off)) {
		err = silofs_verify_off(laddr.off); /* TODO: check ltype */
		if (err) {
			log_err("illegal dir root: ino=%lu off=%ld",
			        ino_of(inode), laddr.off);
			return err;
		}
		if (!ltype_isdtnode(laddr.ltype)) {
			log_err("illegal dir root: ino=%lu off=%ld ltype=%d",
			        ino_of(inode), laddr.off, laddr.ltype);
			return -SILOFS_EFSCORRUPTED;
		}
		dtn_idx = dirin_last_index(dirin);
		if (!dtn_index_isvalid(dtn_idx)) {
			log_err("illegal dir root: ino=%lu dtn_index=%ld",
			        ino_of(inode), (long)dtn_idx);
			return -SILOFS_EFSCORRUPTED;
		}
	} else {
		dtn_idx = dirin_last_index(dirin);
...
{
	const size_t ndents = dirin_ndents(dirin_of(inode));
...
{
	const enum silofs_namehfn hfn = dirin_hashfn(dirin_of(inode));
	ino_t ino;
	int ret;
