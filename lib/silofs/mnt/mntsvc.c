...
static void mntmsg_init(struct silofs_mntmsg *mmsg, enum silofs_mntcmd cmd)
{
	...
	mmsg->mn_magic         = SILOFS_MNTMSG_MAGIC;
	mmsg->mn_version_major = (uint16_t)silofs_sw_vers.major;
	mmsg->mn_version_minor = (uint16_t)silofs_sw_vers.minor;
...
{
	if (mmsg->mn_magic != SILOFS_MNTMSG_MAGIC) {
		return -SILOFS_EINVAL;
	}
	if (mmsg->mn_version_major != silofs_sw_vers.major) {
		return -SILOFS_EPROTO;
	}
	if (mmsg->mn_version_minor > silofs_sw_vers.minor) {
		return -SILOFS_EPROTO;
	}
                                      ...
                                      const struct silofs_mntparams *mntp)
{
	...
	if (mrule->uid != uid_peer) {
		log_info("not allowed to mount: uid=%ld '%s' peer=%s",
		         (long)uid_peer, mntp->path, msvc->ms_peer_ids);
		return -SILOFS_EMOUNT;
	}
	/*
	 * TODO-0048: Support 'ro' mount option.
	 *
	 * Force read-only mode if 'ro=true' is set.
	 */
	return 0;
}
