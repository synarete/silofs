...
static bool bti_has_paddr(const struct silofs_btnode_info *bti,
                          const struct silofs_paddr *paddr)
{
	return silofs_paddr_isequal(paddr, bti_paddr(bti));
}
...
static bool bti_is_leaf(const struct silofs_btnode_info *bti)
{
...
static void
btc_init(struct silofs_btree_ctx *btc, const struct silofs_core_refs *corefs,
...
btc_path_post_split(struct silofs_btree_ctx *btc, size_t parent_idx,
                    struct silofs_btnode_info *bti_nxt, uint64_t key)
{
	if (btc_key(btc) >= key) {
		btc_path_replace_at(btc, parent_idx + 1, bti_nxt);
	}
}
...
{
	return silofs_stage_btnode(btc->corefs, pnptr, out_bti);
}

static void btc_resolve_btroot(const struct silofs_btree_ctx *btc,
                               struct silofs_pnptr *out_pnptr)
{
	silofs_ubi_btroot_of(btc->ubi, btc_vspace(btc), out_pnptr);
}
                            ...
                            struct silofs_btnode_info **out_bti)
{
	...
	btc_resolve_btroot(btc, &pnptr);
	err = btc_stage_btnode(btc, &pnptr, out_bti);
	if (err) {
		return err;
	}
...
static int btc_resolve_child(const struct silofs_btree_ctx *btc,
                             ...
                             struct silofs_pnptr *out_pnptr)
{
	return silofs_bti_resolve(bti, btc_key(btc), out_pnptr);
}
                                  ...
                                  struct silofs_btnode_info **out_bti)
{
	...
	err = btc_resolve_child(btc, parent_bti, &pnptr);
	return_if_err(err);

	err = btc_stage_btnode(btc, &pnptr, &bti);
	return_if_err(err);
...
static int btc_stage_leaf_path(struct silofs_btree_ctx *btc)
{
	int err;
...
static int btc_stage_node_path(struct silofs_btree_ctx *btc,
                               const struct silofs_paddr *paddr)
{
	int err;
	...
	err = btc_stage_path_to_node(btc, paddr);
	return_if_err(err);
                                    ...
                                    struct silofs_btnode_info **out_bti)
{
	...
	err = btc_last_btnode(btc, out_bti);
	return_if_err(err);
                                    ...
                                    struct silofs_btnode_info **out_bti)
{
	...
	err = btc_stage_node_path(btc, paddr);
	return_if_err(err);
...
static int btc_spawn_btnode_at(const struct silofs_btree_ctx *btc,
                               ...
                               struct silofs_btnode_info **out_bti)
{
	return silofs_spawn_btnode(btc->corefs, pnptr, out_bti);
}
                             ...
                             struct silofs_pnptr *out_pnptr)
{
	...
	return silofs_carve_btspace_pnptr(btc->corefs, btc_vspace(btc),
	                                  out_pnptr);
}
...
static void btc_fixup_btroot(const struct silofs_btree_ctx *btc,
                             struct silofs_btnode_info *bti, size_t height)
{
	silofs_bti_set_height(bti, height);
                            ...
                            struct silofs_btnode_info **out_bti)
{
	...
	btc_fixup_btroot(btc, *out_bti, height);
	return 0;
}
...
{
	const struct silofs_pnptr *pnptr_cur  = bti_self(bti_cur);
	const struct silofs_pnptr *pnptr_new  = bti_self(bti_new);
	struct silofs_btnode_info *bti_parent = nullptr;

	bti_parent = btc_path_btnode_at(btc, i);
	return silofs_bti_relink(bti_parent, pnptr_cur, pnptr_new);
}

static void btc_update_btroot(const struct silofs_btree_ctx *btc,
                              struct silofs_btnode_info *bti_new,
                              struct silofs_btnode_info *bti_prv)
{
	silofs_bti_mark_root(bti_new, true);
	if (bti_prv != nullptr) {
		silofs_bti_mark_root(bti_prv, false);
	}
...
static int btc_require_writable_btroot(struct silofs_btree_ctx *btc)
{
	...
	btc_update_btroot(btc, bti_new, nullptr);
...
static void
btc_rebind_btchilds(struct silofs_btree_ctx *btc,
                    struct silofs_btnode_info *bti_parent,
                    struct silofs_btnode_info *bti_left,
                    struct silofs_btnode_info *bti_right, uint64_t key)
{
	const struct silofs_pnptr *left  = bti_self(bti_left);
	const struct silofs_pnptr *right = bti_self(bti_right);

	silofs_rebind_btchilds(bti_parent, left, right, key);
	silofs_unused(btc);
}

static int btc_increase_btree(struct silofs_btree_ctx *btc,
                              struct silofs_btnode_info *bti_cur,
                              struct silofs_btnode_info *bti_nxt, uint64_t key)
{
	...
	btc_rebind_btchilds(btc, bti_root, bti_cur, bti_nxt, key);
	btc_update_btroot(btc, bti_root, bti_cur);
...
static int btc_require_insertable_at(struct silofs_btree_ctx *btc, size_t i)
{
	...
	btc_rebind_btchilds(btc, bti_parent, bti_cur, bti_nxt, key);
...
static int btc_resolve_leaf_by(const struct silofs_btree_ctx *btc,
                               ...
                               struct silofs_pnptr *out_pnptr)
{
	return btc_resolve_child(btc, bti, out_pnptr);
}

static int
btc_resolve_ltop(struct silofs_btree_ctx *btc, struct silofs_pnptr *out_pnptr)
{
	struct silofs_btnode_info *bti = nullptr;
	int err;
...
static int btc_resolve_btleaf(struct silofs_btree_ctx *btc,
                              struct silofs_pnptr *out_pnptr)
{
	struct silofs_btnode_info *bti = nullptr;
	int err;
                              ...
                              struct silofs_pnptr *out_pnptr)
{
	...
	err = btc_resolve_stage_btnode(btc, paddr, &bti);
	return_if_err(err);
                              ...
                              const struct silofs_pnptr *pnptr)
{
	...
	return silofs_bti_insert(bti, btc_key(btc), pnptr);
}

static int
btc_insert_ltop(struct silofs_btree_ctx *btc, const struct silofs_pnptr *pnptr)
{
	int err;
                              ...
                              const struct silofs_pnptr *pnptr)
{
	...
	return silofs_bti_update(bti, btc_key(btc), pnptr);
}

static int
btc_update_ltop(struct silofs_btree_ctx *btc, const struct silofs_pnptr *pnptr)
{
	int err;
...
static int btc_remove_ltop(struct silofs_btree_ctx *btc)
{
	int err;
...
int silofs_resolve_ltop_bpath(const struct silofs_core_refs *corefs,
                              const struct silofs_laddr *laddr,
                              struct silofs_btree_path *out_bpath)
{
	...
	btc_init(&btc, corefs, laddr);
	err = btc_resolve_bpath(&btc, out_bpath);
	btc_fini(&btc);
	return err;
}

int silofs_resolve_ltop_parent(const struct silofs_core_refs *corefs,
                               const struct silofs_laddr *laddr,
                               const struct silofs_paddr *paddr,
                               struct silofs_pnptr *out_pnptr)
{
...
int silofs_resolve_ltop_btleaf(const struct silofs_core_refs *corefs,
                               const struct silofs_laddr *laddr,
                               struct silofs_pnptr *out_pnptr)
{
	struct silofs_btree_ctx btc;
...
int silofs_resolve_ltop_mapping(const struct silofs_core_refs *corefs,
                                const struct silofs_laddr *laddr,
                                struct silofs_pnptr *out_pnptr)
{
	struct silofs_btree_ctx btc;
	...
	btc_init(&btc, corefs, laddr);
	err = btc_resolve_ltop(&btc, out_pnptr);
	btc_fini(&btc);
	return err;
}

int silofs_insert_ltop_mapping(const struct silofs_core_refs *corefs,
                               const struct silofs_laddr *laddr,
                               const struct silofs_pnptr *pnptr)
{
	struct silofs_btree_ctx btc;
	...
	btc_init(&btc, corefs, laddr);
	err = btc_insert_ltop(&btc, pnptr);
	btc_fini(&btc);
	return err;
}

int silofs_update_ltop_mapping(const struct silofs_core_refs *corefs,
                               const struct silofs_laddr *laddr,
                               const struct silofs_pnptr *pnptr)
{
	struct silofs_btree_ctx btc;
	...
	btc_init(&btc, corefs, laddr);
	err = btc_update_ltop(&btc, pnptr);
	btc_fini(&btc);
	return err;
}

int silofs_remove_ltop_mapping(const struct silofs_core_refs *corefs,
                               const struct silofs_laddr *laddr)
{
	struct silofs_btree_ctx btc;
	int err;
