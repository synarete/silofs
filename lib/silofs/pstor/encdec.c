...
static void
calc_aad_of(const struct silofs_exec_ctx *ectx,
            const struct silofs_pnptr *pnptr, struct silofs_caad *out_caad)
{
	calc_aad_by_paddr(ectx->md_hd, &pnptr->paddr, out_caad);
}

static const struct silofs_caad *
caad_by(const struct silofs_exec_ctx *ectx, const struct silofs_pnptr *pnptr,
        const struct silofs_ctag *ctag, struct silofs_caad *caad)
{
	if (ctag == nullptr) {
		caad = nullptr;
	} else {
		calc_aad_of(ectx, pnptr, caad);
	}
	return caad;
}
                         ...
                         struct silofs_ctag *out_ctag)
{
	...
	const struct silofs_encdec_ctx ed_ctx = {
		.ci_hd    = ectx->enc_ci_hd,
		.civ      = &pnptr->nmeta.civkey.iv,
		.ckey     = &pnptr->nmeta.civkey.key,
		.caad     = caad_by(ectx, pnptr, out_ctag, &caad),
		.ctag_in  = nullptr,
		.ctag_out = out_ctag,
                         ...
                         const struct silofs_ctag *ctag)
{
	...
	const struct silofs_pnptr *pnptr      = pni_self(pni);
	const struct silofs_encdec_ctx ed_ctx = {
		.ci_hd    = ectx->dec_ci_hd,
		.civ      = &pnptr->nmeta.civkey.iv,
		.ckey     = &pnptr->nmeta.civkey.key,
		.caad     = caad_by(ectx, pnptr, ctag, &caad),
		.ctag_in  = ctag,
		.ctag_out = nullptr,
