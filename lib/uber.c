                          ...
                          struct silofs_blobref_info **out_bri)
{
	...
	err = silofs_repos_stage_blob(ub_ctx->repos, ub_ctx->repo_mode,
	                              blobid, out_bri);
	if (err && (err != -ENOENT)) {
                            ...
                            struct silofs_ubk_info **out_ubki)
{
	...
	bri_incref(bri);
	err = silofs_repos_spawn_ubk(ub_ctx->repos,
                              ...
                              struct silofs_ubk_info **out_ubki)
{
	...
	bri_incref(bri);
	err = silofs_repos_require_ubk(ub_ctx->repos, ub_ctx->repo_mode,
	                               bkaddr, out_ubki);
	bri_decref(bri);
	return err;
}
                                   ...
                                   struct silofs_sb_info *sbi)
{
	...
	sbi_incref(sbi);
	err = silofs_repos_stage_ubk(ub_ctx->repos, ub_ctx->repo_mode,
	                             sbi_bkaddr(sbi), &ubki);
	if (!err) {
...
static int ubc_stage_ubk_of(const struct silofs_uber_ctx *ub_ctx,
                            ...
                            struct silofs_ubk_info **out_ubki)
{
	return silofs_repos_stage_ubk(ub_ctx->repos, ub_ctx->repo_mode,
	                              bkaddr, out_ubki);
}
