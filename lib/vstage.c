                                 ...
                                 struct silofs_lsid *out_lsid)
{
	...
	vstgc_make_silofs_lsid_of(vstg_ctx, voff, height, ltype, out_lsid);
}
                                ...
                                struct silofs_lsid *out_lsid)
{
	...
	vstgc_make_silofs_lsid_of(vstg_ctx, voff, SILOFS_HEIGHT_VDATA, ltype,
	                          out_lsid);
}
...
vstgc_spawn_super_main_lseg(const struct silofs_vstage_ctx *vstg_ctx)
{
	...
	vstgc_make_silofs_lsid_of_spmaps(vstg_ctx, 0, height, ltype, &lsid);
                             ...
                             struct silofs_spnode_info *sni)
{
	...
	vstgc_make_silofs_lsid_of_spmaps(vstg_ctx, voff, height, ltype, &lsid);
	err = vstgc_spawn_lseg(vstg_ctx, &lsid);
	if (err) {
		return err;
	}
                               ...
                               struct silofs_spleaf_info *sli)
{
	...
	vstgc_make_silofs_lsid_of_vdata(vstg_ctx, voff, ltype, &lsid);
...
{
	return &vstg_ctx->env->base.bootrec->main_ivkey.key;
}
...
static int vstgc_stage_lsmap_for_resolve(struct silofs_vstage_ctx *vstg_ctx)
{
	...
	if (vstg_ctx->vspace != SILOFS_LTYPE_LSMAP) {
		ret = vstgc_stage_lsmap_of(vstg_ctx);
	}
	return ret;
}
...
static void vstgc_rebind_llink(const struct silofs_vstage_ctx *vstg_ctx,
                               const struct silofs_llink *llink)
{
                                  ...
                                  const struct silofs_laddr *src_laddr)
{
	...
	const loff_t voff = vstg_ctx->vaddr->off;
	int err;
	...
	silofs_sli_bind_child(vstg_ctx->sli, voff, &dst_llink.laddr);
	vstgc_rebind_llink(vstg_ctx, &dst_llink);
	return 0;
}
...
static int vstgc_do_stage_lsmap_of(struct silofs_vstage_ctx *vstg_ctx)
{
	struct silofs_vaddr vaddr;

	silofs_vaddr_of_lsmap(&vaddr, vstg_ctx->vspace, vstg_ctx->vaddr->off);
	return stage_lsmap(vstg_ctx->task, &vaddr, vstg_ctx->stg_mode,
	                   &vstg_ctx->lsi);
}
...
static int vstgc_stage_lsmap_of(struct silofs_vstage_ctx *vstg_ctx)
{
	...
	vstgc_increfs(vstg_ctx, SILOFS_HEIGHT_SPLEAF);
	err = vstgc_do_stage_lsmap_of(vstg_ctx);
	vstgc_decrefs(vstg_ctx, SILOFS_HEIGHT_SPLEAF);
	return err;
}
                                ...
                                struct silofs_vaddrs *out_vaddrs)
{
	...
	err = vstgc_stage_lsmap_of(vstg_ctx);
	if (err) {
		return err;
	}
