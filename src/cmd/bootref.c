...
static void cmd_load_bref_file(const char *pathname, char **out_txt)
{
	...
	if (len >= SILOFS_KILO) {
		silofs_die(-EFBIG, "illegal boot-ref file: %s", pathname);
	}
	err = silofs_sys_open(pathname, O_RDONLY, 0, &fd);
	if (err) {
		silofs_die(err, "failed to open boot-ref: %s", pathname);
	}
	txt = cmd_zalloc(len + 1);
	err = silofs_sys_readn(fd, txt, len);
	if (err) {
		silofs_die(err, "failed to read boot-ref: %s", pathname);
	}
	silofs_sys_close(fd);
	*out_txt = txt;
}
...
static void cmd_save_bref_file(const char *pathname, const char *txt)
{
	...
	if (err) {
		silofs_die(err, "failed to create boot-ref: %s", pathname);
	}
	err = silofs_sys_fchmod(fd, S_IRUSR | S_IRGRP);
	if (err) {
		silofs_die(err, "failed to change-mode of: %s", pathname);
	}
	err = silofs_sys_writen(fd, txt, len);
	if (err) {
		silofs_die(err, "failed to write boot-ref: %s", pathname);
	}
...
static void cmd_decode_bootref(struct silofs_fs_bref *bref, const char *txt)
{
	...
	if (!silofs_substr_isascii(&ss)) {
		silofs_die(0, "non-ascii character in: %s", bref->name);
	}
	if (ss.len >= sizeof(sbuf.str)) {
		silofs_die(0, "illegal boot-ref length in: %s", bref->name);
	}
	silofs_strbuf_setup(&sbuf, &ss);
	err = silofs_bootref_import(bref, &sbuf);
	if (err) {
		silofs_die(err, "bad boot-ref in: %s", bref->name);
	}
}
                               ...
                               enum silofs_ctype ctype)
{
	if (bref->caddr.ctype != ctype) {
		if (ctype == SILOFS_CTYPE_BOOTREC) {
			silofs_die(0, "not fs boot-ref: %s", bref->name);
		} else if (ctype == SILOFS_CTYPE_PACKIDX) {
			silofs_die(0, "not archive boot-ref: %s", bref->name);
		} else {
			silofs_die(0, "bad boot-ref: %s", bref->name);
		}
	}
}
