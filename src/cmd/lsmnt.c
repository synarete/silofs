...
static void lsmnt_getopt(void)
{
	...
	while (opt_chr > 0) {
		opt_chr = silofs_getopt_subcmd("lh", opts);
		if (opt_chr == 'l') {
			silofs_globals.cmd.lsmnt.long_listing = true;
		} else if (opt_chr == 'h') {
...
static void lsmnt_finalize(void)
{
	...
	silofs_pfree_string(&silofs_globals.cmd.lsmnt.mntpoint_real);
}
...
static void lsmnt_setup_check_params(void)
{
	...
	path = silofs_globals.cmd.lsmnt.mntpoint;
	if (path != NULL) {
		...
		silofs_globals.cmd.lsmnt.mntpoint_real =
		        silofs_realpath_safe(path);
	}
}
...
static void lsmnt_parse_mountinfo_line(const struct silofs_substr *line)
{
	...
	if (silofs_globals.cmd.lsmnt.long_listing) {
