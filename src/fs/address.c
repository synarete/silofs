...
static uint64_t silofs_cpu_to_voff_stype(loff_t voff, enum silofs_stype stype)
{
	uint64_t voff_stype;
...
static void silofs_voff_stype_to_cpu(uint64_t voff_stype, loff_t *out_voff,
...
static void oaddr_setup_by(struct silofs_oaddr *oaddr,
                           const struct silofs_blobid *bid,
                           enum silofs_stype stype, loff_t bpos)
{
	silofs_oaddr_setup(oaddr, bid, stype_size(stype), bpos);
}
...
{
	oaddr_setup_by(&uaddr->oaddr, bid, stype, bpos);
	uaddr->voff = voff;
	uaddr->stype = stype;
}
                         ...
                         const struct silofs_uaddr *uaddr)
{
	...
	uaddr56->voff_stype =
	        silofs_cpu_to_voff_stype(uaddr->voff, uaddr->stype);
}
                           ...
                           struct silofs_uaddr *uaddr)
{
	...
	silofs_voff_stype_to_cpu(uaddr56->voff_stype,
...
void silofs_vaddr64_set(struct silofs_vaddr64 *vadr,
                        const struct silofs_vaddr *vaddr)
{
	vadr->voff_stype = silofs_cpu_to_voff_stype(vaddr->voff, vaddr->stype);
}
                          ...
                          struct silofs_vaddr *vaddr)
{
	...
	silofs_voff_stype_to_cpu(vadr->voff_stype, &voff, &stype);
