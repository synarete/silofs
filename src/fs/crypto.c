...
static int mdigest_init(struct silofs_mdigest *md)
{
	int algo;
	gcry_error_t err;
	const int algos[] = {
...
static void mdigest_fini(struct silofs_mdigest *md)
{
	if (md->md_hd != NULL) {
...
static void passphrase_setup(struct silofs_passphrase *pp,
...
int silofs_passphrase_setup(struct silofs_passphrase *pp, const void *pass)
{
...
void silofs_passphrase_reset(struct silofs_passphrase *pp)
{
	silofs_memzero(pp, sizeof(*pp));
	pp->passlen = 0;
}

static int passphrase_check(const struct silofs_passphrase *pp)
{
...
static int derive_iv(const struct silofs_kdf_desc *kdf,
                     const struct silofs_passphrase *pp,
