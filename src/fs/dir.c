...
static int recheck_dnode(const struct silofs_dir_ctx *d_ctx,
                         struct silofs_dnode_info *dni)
{
	ino_t h_ino;
	ino_t d_ino;
...
static int dic_do_stage_dnode(const struct silofs_dir_ctx *d_ctx,
                              const struct silofs_vaddr *vaddr,
                              struct silofs_dnode_info **out_dni)
{
	...
	err = silofs_stage_vnode(d_ctx->sbi, vaddr, d_ctx->stg_flags, &vi);
	if (err) {
		return err;
	}
	dni = silofs_dni_from_vi(vi);
	silofs_dni_rebind_view(dni);
	err = recheck_dnode(d_ctx, dni);
	if (err) {
		return err;
	}
	*out_dni = dni;
                           ...
                           struct silofs_dnode_info **out_dni)
{
	...
	err = dic_do_stage_dnode(d_ctx, vaddr, out_dni);
                           ...
                           struct silofs_dnode_info **out_dni)
{
	...
	err = dic_stage_dnode(d_ctx, vaddr, out_dni);
                                 ...
                                 struct silofs_dir_entry_info *dei)
{
	...
	depth = dtn_depth(dni->dtn);
	while (depth_isvalid(depth)) {
		err = search_dnode(dni, name, dei);
		if (!err) {
			return 0;
		}
		if (err != -ENOENT) {
		...
		err = dic_stage_child_by_name(d_ctx, dni, &child_dni);
		if (err) {
			return err;
		}
		dni = child_dni;
		err = dni_check_child_depth(dni, depth);
		if (err) {
			return err;
		}
		depth++;
                              ...
                              struct silofs_dir_entry_info *dei)
{
	...
	dni_incref(root_dni);
	err = dic_do_lookup_by_tree(d_ctx, root_dni, dei);
	dni_decref(root_dni);
	return err;
}
...
static int dic_do_lookup_dentry(struct silofs_dir_ctx *d_ctx,
                                struct silofs_ino_dt *ino_dt)
{
	int err;
	struct silofs_dir_entry_info dei;
                         ...
                         struct silofs_ino_dt *out_idt)
{
	...
	ii_incref(dir_ii);
	err = dic_do_lookup_dentry(&d_ctx, out_idt);
	ii_decref(dir_ii);
	return err;
}
                                   ...
                                   struct silofs_dnode_info **out_dni)
{
	...
	size_t child_index;
	const struct silofs_qstr *name = d_ctx->name;
	const struct silofs_vaddr *vaddr = dni_vaddr(parent_dni);

	parent_index = dtn_node_index(parent_dni->dtn);
	child_index = hash_to_child_index(name->hash, parent_index);
	err = dic_spawn_child(d_ctx, vaddr, child_index, out_dni);
	if (err) {
		return err;
	}
	dni_bind_child_at(parent_dni, dni_vaddr(*out_dni), child_index);
	return 0;
}
                                ...
                                struct silofs_dnode_info **out_dni)
{
	...
	dni_incref(parent_dni);
	err = dic_do_spawn_bind_child(d_ctx, parent_dni, out_dni);
	dni_decref(parent_dni);
	return err;
}
...
static int dic_do_add_to_tree(const struct silofs_dir_ctx *d_ctx,
                              struct silofs_dnode_info *root_dni)
{
	int err;
	size_t depth;
	...
	depth = dtn_depth(dni->dtn);
	while (depth_isvalid(depth)) {
		err = dic_add_to_dnode(d_ctx, dni);
		if (!err) {
			return 0;
		}
		err = dic_require_child(d_ctx, dni, &dni);
		if (err) {
			return err;
		}
		err = dni_check_child_depth(dni, depth);
                           ...
                           struct silofs_dnode_info *root_dni)
{
	...
	dni_incref(root_dni);
	err = dic_do_add_to_tree(d_ctx, root_dni);
	dni_decref(root_dni);
	return err;
}
...
static int
dic_do_stage_child_by_ord(struct silofs_dir_ctx *d_ctx,
                          const struct silofs_dnode_info *dni, size_t ord,
                          struct silofs_dnode_info **out_dni)
{
	int err;
	struct silofs_vaddr vaddr;
                                  ...
                                  struct silofs_dnode_info **out_dni)
{
	...
	dni_incref(dni);
	err = dic_do_stage_child_by_ord(d_ctx, dni, ord, out_dni);
	dni_decref(dni);
	return err;
}
                                     ...
                                     const struct silofs_dnode_info *root_dni)
{
	...
	while (!err && !dic_stopped(d_ctx)) {
		if (!dic_isindex_inrange(d_ctx, index)) {
			return dic_readdir_eos(d_ctx);
		}
		err = dic_stage_node_by_index(d_ctx, root_dni, index, &dni);
		if (err == -ENOENT) {
			index++;
			err = 0;
			continue;
		...
		err = dic_iterate_node(d_ctx, dni);
		if (err) {
			break;
		}
		err = dic_next_node(d_ctx, dni, &index);
	}
	return err;
}
                                  ...
                                  struct silofs_dnode_info *root_dni)
{
	...
	dni_incref(root_dni);
	err = dic_do_iterate_tree_nodes(d_ctx, root_dni);
	dni_decref(root_dni);
	return err;
}
...
static int dic_do_readdir(struct silofs_dir_ctx *d_ctx)
{
	int err;

	err = dic_check_dir_io(d_ctx);
	if (err) {
		return err;
	}
	err = dic_check_raccess(d_ctx);
	if (err) {
		return err;
	}
	err = dic_check_stage_parent(d_ctx);
	if (err) {
		return err;
	}
	err = dic_readdir_emit(d_ctx);
	if (err) {
		return err;
	}
	return 0;
...
static int dic_do_discard_childs_of(struct silofs_dir_ctx *d_ctx,
                                    struct silofs_dnode_info *dni)
{
	int err = 0;
                                 ...
                                 struct silofs_dnode_info *dni)
{
	...
	dni_incref(dni);
	err = dic_do_discard_childs_of(d_ctx, dni);
	dni_decref(dni);
	return err;
}
                               ...
                               const struct silofs_dir_entry_info *dei)
{
	...
	return !dir_ndents(dir_ii) ? dic_finalize_tree(d_ctx) : 0;
}
                            ...
                            const struct silofs_dir_entry_info *dei)
{
	...
	err = dic_do_erase_dentry(d_ctx, dei);
                         ...
                         const struct silofs_qstr *name)
{
	...
	ii_incref(dir_ii);
	err = dic_do_remove_dentry(&d_ctx);
	ii_decref(dir_ii);
	return err;
}
