...
static const struct silofs_uaddr *sbi_uaddr(const struct silofs_sb_info *sbi)
{
	return ui_uaddr(&sbi->s_ui);
}
...
static int fse_init_qalloc(struct silofs_fs_env *fse)
{
	...
	err = silofs_boot_memsize(fse->fs_args.memwant, &memsize);
	if (err) {
		return err;
	}
...
static struct silofs_sb_info *fse_sbi(const struct silofs_fs_env *fse)
{
	...
	silofs_assert_not_null(fse->fs_apex->fa_sbi);

	return fse->fs_apex->fa_sbi;
}
...
{
	if (fse->fs_apex && fse->fs_apex->fa_sbi) {
		silofs_drop_itable_cache(fse->fs_apex->fa_sbi);
	}
	silofs_cache_drop(fse->fs_cache);
	silofs_burnstack();
}
...
static int fse_shut_super(struct silofs_fs_env *fse)
{
	...
	if ((apex == NULL) || (apex->fa_sbi == NULL)) {
		return 0;
	}
	err = silofs_sbi_shut(apex->fa_sbi);
	if (err) {
		return err;
	}
...
static int fse_derive_main_key(struct silofs_fs_env *fse)
{
	...
	err = silofs_mbr_cipher_args(fse->fs_apex->fa_mbr, &cip_args);
	if (err) {
...
static int fse_reload_super(struct silofs_fs_env *fse)
{
	...
	err = silofs_apex_resolve_sb_addr(fse->fs_apex, &uaddr);
	if (err) {
		return err;
	}
	err = silofs_apex_stage_super(fse->fs_apex, &uaddr, &sbi);
	if (err) {
		return err;
	}
                              ...
                              struct silofs_oper *op)
{
	...
	op->ucred.uid = fse->fs_args.uid;
	op->ucred.gid = fse->fs_args.gid;
	op->ucred.pid = fse->fs_args.pid;
	op->ucred.umask = fse->fs_args.umask;
	op->unique = 0; /* TODO: make me a negative running sequence no */

	return silofs_ts_gettime(&op->xtime, true);
}
                            ...
                            const struct silofs_oper *op)
{
	...
	err = silofs_apex_spawn_super(fse->fs_apex, &sbi);
	if (err) {
		return err;
	}
	err = fse_check_sb(fse, sbi->sb);
	...
	silofs_sbi_update_birth_time(sbi, op->xtime.tv_sec);
	silofs_apex_bind_to_sbi(fse->fs_apex, sbi);
	silofs_apex_rebind_sb_addr(fse->fs_apex, sbi_uaddr(sbi));
	return 0;
}
...
int silofs_fse_serve(struct silofs_fs_env *fse)
{
	...
	err = silofs_fse_reopen(fse);
	if (err) {
		log_err("failed to lock repo: %s %d", repodir, err);
		return err;
	}
	err = silofs_fse_reload(fse);
	if (err) {
		log_err("load-fs error: %s %d", repodir, err);
		goto out;
	}
	...
	if (err) {
		log_err("term-fs error: %s %d", fse->fs_args.mntdir, err);
		goto out;
	}
out:
	fse_close_repo(fse);
