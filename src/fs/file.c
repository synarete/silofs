...
static loff_t off_max_min(loff_t off1, loff_t off2, loff_t off3)
{
	return off_min(off_max(off1, off2), off3);
}

static ssize_t data_size_of(enum silofs_stype stype)
{
	return stype_ssize(stype);
}
...
{
	const ssize_t len = data_size_of(stype);
...
static bool vaddr_isdatabk(const struct silofs_vaddr *vaddr)
{
...
/* a legacy code from days where blobs where mmapped */
static bool fli_has_mmdata(const struct silofs_fileaf_info *fli)
{
	silofs_unused(fli);
	return false;
}
...
static void *address_within_bk(void *bk_start, loff_t off_in_bk)
{
                          ...
                          struct silofs_fiovec *out_fiov)
{
	...
	const struct silofs_repo *repo = f_ctx->apex->fa_repo;

	err = silofs_resolve_oaddr(f_ctx->sbi, vaddr, &oaddr);
	if (err) {
		return err;
	}
	oaddr.pos += off_within;
	oaddr.len = len;
	...
	err = silofs_bli_resolve(bli, &oaddr, out_fiov);
	if (err) {
		return err;
	}
	return 0;
...
{
	return off_is_inrange(pos, ftn_beg(ftn), ftn_end(ftn));
}
...
ftn_span_by_height(const struct silofs_ftree_node *ftn, size_t height)
{
	...
	silofs_assert_eq(1L << fm_shift, ftn_nchilds_max(ftn));
...
static void fmc_init(struct silofs_fmap_ctx *fm_ctx,
                     const struct silofs_file_ctx *f_ctx,
                     const struct silofs_vaddr *vaddr, loff_t file_pos)
{
	silofs_assert_not_null(vaddr);
...
static void fmc_setup(struct silofs_fmap_ctx *fm_ctx,
                      const struct silofs_file_ctx *f_ctx,
                      const struct silofs_vaddr *vaddr,
                      struct silofs_finode_info *parent,
                      size_t slot, loff_t file_pos)
{
	const bool bottom = (parent != NULL) && fni_isbottom(parent);

	fmc_init(fm_ctx, f_ctx, vaddr, file_pos);
	fm_ctx->parent = parent;
	fm_ctx->slot_idx = slot;
	if (fm_ctx->head1 || fm_ctx->head2 || vaddr_isdata(vaddr) || bottom) {
		fm_ctx->leaf = true;
...
static void fmc_nomap(struct silofs_fmap_ctx *fm_ctx,
                      const struct silofs_file_ctx *f_ctx,
                      struct silofs_finode_info *parent, loff_t file_pos)
{
	fmc_setup(fm_ctx, f_ctx, vaddr_none(), parent, UINT_MAX, file_pos);
}
...
static int fmc_require_mutable(const struct silofs_fmap_ctx *fm_ctx)
{
	...
	return silofs_sbi_require_mutable_vaddr(sbi, vaddr);
}
...
static void fic_resolve_child_at(const struct silofs_file_ctx *f_ctx,
                                 ...
                                 struct silofs_fmap_ctx *out_fm_ctx)
{
	struct silofs_vaddr vaddr;

	fni_resolve_child_by_slot(fni, slot, &vaddr);
	fmc_setup(out_fm_ctx, f_ctx, &vaddr, fni, slot, file_pos);
}
                              ...
                              struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	if (fni != NULL) {
		child_slot = fni_child_slot_of(fni, file_pos);
		fic_resolve_child_at(f_ctx, fni, file_pos,
		                     child_slot, out_fm_ctx);
	} else {
		fmc_init(out_fm_ctx, f_ctx, vaddr_none(), file_pos);
	}
}
...
static bool fic_has_more_io(const struct silofs_file_ctx *f_ctx)
{
	return (f_ctx->off < f_ctx->end) &&
	       !f_ctx->fm_stop && !f_ctx->op->interrupt;
}
...
{
	return head1_off_end(SILOFS_FILE_HEAD1_NLEAVES - 1);
}
...
{
	return head2_off_end(SILOFS_FILE_HEAD2_NLEAVES - 1);
}
...
static bool fic_has_partial_write_at(const struct silofs_file_ctx *f_ctx,
                                     const struct silofs_vaddr *vaddr)
{
	return off_is_partial(f_ctx->off, f_ctx->end, vaddr_stype(vaddr));
}

static bool fmc_has_partial_write(const struct silofs_fmap_ctx *fm_ctx)
{
	return fic_has_partial_write_at(fm_ctx->f_ctx, &fm_ctx->vaddr);
}
...
static size_t off_to_data_size(loff_t off)
{
	return (size_t)data_size_of(off_to_data_stype(off));
}
                                   ...
                                   struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	fmc_setup(out_fm_ctx, f_ctx, &vaddr, NULL, slot, f_ctx->off);
}
...
static size_t fic_head2_leaf_slot_of(const struct silofs_file_ctx *f_ctx)
{
	...
	silofs_assert_lt(off, SILOFS_BK_SIZE);
	silofs_assert_ge(off, head1_off_max());
...
{
	int err;
	const size_t len_max = SILOFS_IO_SIZE_MAX;
...
static int fmc_probe_unwritten(const struct silofs_fmap_ctx *fm_ctx,
                               bool *out_res)
{
	return fic_probe_unwritten_at(fm_ctx->f_ctx, &fm_ctx->vaddr, out_res);
}
...
static int fic_do_stage_fileaf(const struct silofs_file_ctx *f_ctx,
                               const struct silofs_vaddr *vaddr,
                               struct silofs_fileaf_info **out_fli)
{
	...
	err = silofs_stage_vnode(f_ctx->sbi, vaddr, f_ctx->stg_flags, &vi);
	if (err) {
		return err;
	}
                            ...
                            struct silofs_fileaf_info **out_fli)
{
	...
	ii_incref(f_ctx->ii);
	err = fic_do_stage_fileaf(f_ctx, vaddr, out_fli);
	ii_decref(f_ctx->ii);
	return err;
}
...
static int fic_zero_data_leaf_at(const struct silofs_file_ctx *f_ctx,
                                 const struct silofs_vaddr *vaddr)
{
	const ssize_t len = data_size_of(vaddr_stype(vaddr));

	silofs_assert_eq(len, vaddr->len);

	return fic_zero_data_leaf_range(f_ctx, vaddr, 0, (size_t)len);
}

static int fmc_stage_fileaf_at(const struct silofs_fmap_ctx *fm_ctx,
                               struct silofs_fileaf_info **out_fli)
{
	int err;

	silofs_assert(!fic_has_kcopy_mode(fm_ctx->f_ctx));

	*out_fli = NULL;
	if (!fm_ctx->has_data) {
		return -ENOENT;
	}
	err = fic_stage_fileaf(fm_ctx->f_ctx, &fm_ctx->vaddr, out_fli);
	if (err) {
		return err;
	}
	return 0;
...
static int fic_do_stage_finode(const struct silofs_file_ctx *f_ctx,
                               const struct silofs_vaddr *vaddr,
                            ...
                            struct silofs_finode_info **out_fni)
{
	...
	ii_incref(f_ctx->ii);
	err = fic_do_stage_finode(f_ctx, vaddr, out_fni);
	ii_decref(f_ctx->ii);
	return err;
}
                           ...
                           struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_seek_tree_recursive(f_ctx, fni, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
...
static int
fic_do_seek_tree_recursive(struct silofs_file_ctx *f_ctx,
                           struct silofs_finode_info *parent_fni,
                           struct silofs_fmap_ctx *out_fm_ctx)
{
	int err;
	...
	if (fni_isbottom(parent_fni)) {
		return fic_seek_tree_at_leaves(f_ctx, parent_fni, out_fm_ctx);
	}
	err = fic_is_seek_hole(f_ctx) ? 0 : -ENOENT;
	start_slot = fni_child_slot_of(parent_fni, f_ctx->off);
	for (size_t slot = start_slot; slot < nslots_max; ++slot) {
		err = fic_seek_tree_recursive_at(f_ctx, parent_fni,
		                                 slot, out_fm_ctx);
		if (err != -ENOENT) {
			break;
		}
                                   ...
                                   struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_do_seek_tree_recursive(f_ctx, parent_fni, out_fm_ctx);
                                ...
                                struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_seek_tree_recursive(f_ctx, root_fni, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
                                        ...
                                        struct silofs_fmap_ctx *out_fm_ctx)
{
	while (fic_has_head1_leaves_io(f_ctx)) {
		fic_resolve_head1_leaf(f_ctx, out_fm_ctx);
	...
	while (fic_has_head2_leaves_io(f_ctx)) {
		fic_resolve_head2_leaf(f_ctx, out_fm_ctx);
		if (out_fm_ctx->has_data) {
			return 0;
		}
		fic_advance_to_next(f_ctx);
...
static void fic_child_of_current_pos(const struct silofs_file_ctx *f_ctx,
                                     ...
                                     struct silofs_fmap_ctx *out_fm_ctx)
{
	fic_resolve_child(f_ctx, parent_fni, f_ctx->off, out_fm_ctx);
}
...
static void fic_resolve_tree_leaf(const struct silofs_file_ctx *f_ctx,
                                  ...
                                  struct silofs_fmap_ctx *out_fm_ctx)
{
	fic_child_of_current_pos(f_ctx, parent_fni, out_fm_ctx);
}
                                 ...
                                 struct silofs_finode_info **out_fni)
{
	...
	size_t height;
	struct silofs_finode_info *fni = NULL;
	struct silofs_fmap_ctx fm_ctx = { .file_pos = -1 };
	...
	while (height--) {
		...
		fic_resolve_curr_node(f_ctx, fni, &fm_ctx);
		err = fic_stage_finode(f_ctx, &fm_ctx.vaddr, &fni);
		if (err) {
			return err;
		}
	}
...
static int
fic_do_read_by_copy_from_fileaf(struct silofs_file_ctx *f_ctx,
                                struct silofs_fileaf_info *fli, size_t *out_sz)
{
	return fic_export_data_by_fileaf(f_ctx, fli, out_sz);
}

static int fic_read_fileaf_by_copy(struct silofs_file_ctx *f_ctx,
                                   struct silofs_fileaf_info *fli,
                                   size_t *out_sz)
{
	int err;
	...
	fli_incref(fli);
	err = fic_do_read_by_copy_from_fileaf(f_ctx, fli, out_sz);
	fli_decref(fli);
...
fic_read_leaf_at(struct silofs_file_ctx *f_ctx,
                 const struct silofs_fmap_ctx *fm_ctx, size_t *out_sz)
{
	return fic_export_data_by_vaddr(f_ctx, &fm_ctx->vaddr, out_sz);
}
...
static int fic_read_from_leaf(struct silofs_file_ctx *f_ctx,
                              const struct silofs_fmap_ctx *fm_ctx,
                              size_t *out_len)
{
	int err;
	...
	*out_len = 0;
	err = fmc_probe_unwritten(fm_ctx, &unwritten);
	if (err) {
		return err;
	}
	if (unwritten) {
		...
		err = fic_read_leaf_at(f_ctx, fm_ctx, out_len);
		if (err) {
			return err;
		}
	} else {
		...
		err = fic_read_fileaf_by_copy(f_ctx, fli, out_len);
		if (err) {
			return err;
		}
	}
...
static int fic_do_read_tree_leaves(struct silofs_file_ctx *f_ctx,
                                   struct silofs_finode_info *parent_fni)
{
	int err;
	size_t len;
	struct silofs_fmap_ctx fm_ctx;

	while (fic_has_more_io(f_ctx)) {
		fic_resolve_tree_leaf(f_ctx, parent_fni, &fm_ctx);
		err = fic_read_from_leaf(f_ctx, &fm_ctx, &len);
		if (err) {
			return err;
		}
...
static int fic_read_tree_leaves(struct silofs_file_ctx *f_ctx,
                                struct silofs_finode_info *parent_fni)
{
	int err;

	fni_incref(parent_fni);
	err = fic_do_read_tree_leaves(f_ctx, parent_fni);
	fni_decref(parent_fni);
...
static int fic_read_by_tree_map(struct silofs_file_ctx *f_ctx)
{
	...
	while (fic_has_more_io(f_ctx)) {
		...
		err = fic_read_tree_leaves(f_ctx, parent_fni);
		if (err) {
			return err;
		}
	}
...
static int fic_read_by_head_leaves(struct silofs_file_ctx *f_ctx)
{
	...
	while (fic_has_head1_leaves_io(f_ctx)) {
		fic_resolve_head1_leaf(f_ctx, &fm_ctx);
		err = fic_read_from_leaf(f_ctx, &fm_ctx, &len);
		if (err) {
			return err;
		}
	...
	while (fic_has_head2_leaves_io(f_ctx)) {
		fic_resolve_head2_leaf(f_ctx, &fm_ctx);
		err = fic_read_from_leaf(f_ctx, &fm_ctx, &len);
		if (err) {
			return err;
		}
...
static int fic_do_read_iter(struct silofs_file_ctx *f_ctx)
{
	int err;
...
static int fmc_clear_unwritten_by(const struct silofs_fmap_ctx *fm_ctx)
{
	return fic_clear_unwritten_at(fm_ctx->f_ctx, &fm_ctx->vaddr);
}
                                ...
                                struct silofs_vaddr *out_vaddr)
{
	...
	err = silofs_claim_vspace(f_ctx->sbi, stype, &ova);
	if (err) {
		return err;
	}
...
static void fmc_update_head1_leaf(const struct silofs_fmap_ctx *fm_ctx)
{
	fic_set_head1_leaf_at(fm_ctx->f_ctx, fm_ctx->slot_idx, &fm_ctx->vaddr);
	ii_dirtify(fm_ctx->f_ctx->ii);
}

static void fmc_update_head2_leaf(const struct silofs_fmap_ctx *fm_ctx)
{
	fic_set_head2_leaf_at(fm_ctx->f_ctx, fm_ctx->slot_idx, &fm_ctx->vaddr);
	ii_dirtify(fm_ctx->f_ctx->ii);
}
                                       ...
                                       struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	fmc_setup(out_fm_ctx, f_ctx, &vaddr, NULL, slot, f_ctx->off);
	fmc_update_head1_leaf(out_fm_ctx);
	return 0;
}
                                       ...
                                       struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	fmc_setup(out_fm_ctx, f_ctx, &vaddr, NULL, slot, f_ctx->off);
	fmc_update_head2_leaf(out_fm_ctx);
	return 0;
}
                                      ...
                                      struct silofs_vaddr *out_vaddr)
{
	...
	fni_incref(parent_fni);
	err = fic_create_data_leaf(f_ctx, SILOFS_STYPE_DATABK, out_vaddr);
	if (!err) {
		fni_bind_child(parent_fni, f_ctx->off, out_vaddr);
	}
	fni_decref(parent_fni);
	return err;
}
...
static int
fic_do_require_finode(const struct silofs_file_ctx *f_ctx,
                      struct silofs_finode_info *parent_fni,
                      struct silofs_finode_info **out_fni)
{
	int err;
	...
	fic_resolve_curr_node(f_ctx, parent_fni, &fm_ctx);
	if (fm_ctx.has_target) {
		err = fic_stage_finode(f_ctx, &fm_ctx.vaddr, out_fni);
	} else {
		err = fic_spawn_bind_finode(f_ctx, parent_fni, out_fni);
	}
	return err;
}
                              ...
                              struct silofs_finode_info **out_fni)
{
	...
	fni_incref(parent_fni);
	err = fic_do_require_finode(f_ctx, parent_fni, out_fni);
	fni_decref(parent_fni);
	return err;
}
                                 ...
                                 struct silofs_finode_info **out_fni)
{
	...
	*out_fni = NULL;
	...
	return unlikely(*out_fni == NULL) ? -EFSCORRUPTED : 0;
}
                  ...
                  const struct silofs_fmap_ctx *fm_ctx, size_t *out_sz)
{
	...
	err = fic_import_data_by_vaddr(f_ctx, &fm_ctx->vaddr, out_sz);
	if (err) {
		return err;
	}
	err = fic_clear_unwritten_at(f_ctx, &fm_ctx->vaddr);
	if (err) {
		return err;
	}
	return 0;
...
static int fmc_prepare_unwritten_leaf(const struct silofs_fmap_ctx *fm_ctx)
{
	int err;
	bool partial;
	bool unwritten;

	partial = fmc_has_partial_write(fm_ctx);
	if (!partial) {
		return 0;
	}
	err = fmc_probe_unwritten(fm_ctx, &unwritten);
	...
	err = fic_zero_data_leaf_at(fm_ctx->f_ctx, &fm_ctx->vaddr);
	if (err) {
		return err;
	}
	return 0;
...
{
	int err;
	struct silofs_vaddr *vaddr = &out_fm_ctx->vaddr;

	fic_resolve_tree_leaf(f_ctx, parent_fni, out_fm_ctx);
	if (out_fm_ctx->has_data) {
		return fmc_require_mutable(out_fm_ctx);
	}
	err = fic_create_tree_leaf_space(f_ctx, parent_fni, vaddr);
	if (err) {
		return err;
	}
...
static int
fmc_write_to_leaf(const struct silofs_fmap_ctx *fm_ctx, size_t *out_len)
{
	int err;
	struct silofs_fileaf_info *fli = NULL;

	*out_len = 0;
	err = fmc_prepare_unwritten_leaf(fm_ctx);
	if (err) {
		return err;
	}
	if (fic_has_kcopy_mode(fm_ctx->f_ctx)) {
		err = fic_write_leaf_at(fm_ctx->f_ctx, fm_ctx, out_len);
		if (err) {
			return err;
		}
	} else {
		...
		err = fic_write_leaf_by_copy(fm_ctx->f_ctx, fli, out_len);
		if (err) {
			return err;
		}
	}
...
static int fic_do_write_tree_leaves(struct silofs_file_ctx *f_ctx,
                                    struct silofs_finode_info *parent_fni)
{
	int err;
	size_t len;
	...
	while (fic_has_more_io(f_ctx)) {
		err = fic_require_tree_leaf(f_ctx, parent_fni, &fm_ctx);
		if (err) {
			return err;
		}
		err = fmc_write_to_leaf(&fm_ctx, &len);
		if (err) {
			return err;
		}
		fic_advance_by_nbytes(f_ctx, len);
...
static int fic_write_to_leaves(struct silofs_file_ctx *f_ctx,
                               struct silofs_finode_info *fni)
{
	int err;

	fni_incref(fni);
	err = fic_do_write_tree_leaves(f_ctx, fni);
	fni_decref(fni);
	return err;
}
...
static int fic_write_by_tree_map(struct silofs_file_ctx *f_ctx)
{
	...
	while (fic_has_more_io(f_ctx)) {
		...
		err = fic_write_to_leaves(f_ctx, fni);
		if (err) {
			return err;
		}
	}
                                  ...
                                  struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_create_head1_leaf_space(f_ctx, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
                                  ...
                                  struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_create_head2_leaf_space(f_ctx, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
...
static int fic_write_by_head_leaves(struct silofs_file_ctx *f_ctx)
{
	...
	while (fic_has_head1_leaves_io(f_ctx)) {
		err = fic_require_head1_leaf(f_ctx, &fm_ctx);
		if (err) {
			return err;
		}
		err = fmc_write_to_leaf(&fm_ctx, &len);
	...
	while (fic_has_head2_leaves_io(f_ctx)) {
		err = fic_require_head2_leaf(f_ctx, &fm_ctx);
		if (err) {
			return err;
		}
		err = fmc_write_to_leaf(&fm_ctx, &len);
...
static int fic_do_write_iter(struct silofs_file_ctx *f_ctx)
{
	int err;
...
static int fic_do_drop_recursive(struct silofs_file_ctx *f_ctx,
                                 struct silofs_finode_info *fni)
{
	int err;
                              ...
                              struct silofs_finode_info *fni)
{
	...
	fni_incref(fni);
	err = fic_do_drop_recursive(f_ctx, fni);
	fni_decref(fni);
	return err;
}
...
int silofs_drop_reg(struct silofs_inode_info *ii)
{
	...
	ii_incref(ii);
	err = fic_drop_data_and_meta(&f_ctx);
	ii_decref(ii);
	return err;
}
...
static int fmc_do_discard_partial(const struct silofs_fmap_ctx *fm_ctx)
{
	const struct silofs_vaddr *vaddr = &fm_ctx->vaddr;
	const enum silofs_stype stype = vaddr_stype(vaddr);
	const loff_t off = fm_ctx->file_pos;
	const loff_t blobid = off_in_data(off, stype);
	const size_t len = len_of_data(off, fm_ctx->f_ctx->end, stype);

	return fic_zero_data_leaf_range(fm_ctx->f_ctx, vaddr, blobid, len);
}

static int fmc_discard_partial(const struct silofs_fmap_ctx *fm_ctx)
{
	int err;

	fni_incref(fm_ctx->parent);
	err = fmc_do_discard_partial(fm_ctx);
	fni_decref(fm_ctx->parent);
	return err;
}

static int fmc_discard_data_leaf_at(const struct silofs_fmap_ctx *fm_ctx)
{
	int err;

	fni_incref(fm_ctx->parent);
	err = fic_discard_data_leaf(fm_ctx->f_ctx, &fm_ctx->vaddr);
	fni_decref(fm_ctx->parent);
	return err;
}

static int fmc_discard_entire(const struct silofs_fmap_ctx *fm_ctx)
{
	int err;

	err = fmc_discard_data_leaf_at(fm_ctx);
	if (err) {
		return err;
	}
	if (off_is_head1(fm_ctx->file_pos)) {
		silofs_assert_null(fm_ctx->parent);
		fic_reset_head1_leaf_at(fm_ctx->f_ctx, fm_ctx->slot_idx);
	} else if (off_is_head2(fm_ctx->file_pos)) {
		silofs_assert_null(fm_ctx->parent);
		fic_reset_head2_leaf_at(fm_ctx->f_ctx, fm_ctx->slot_idx);
	} else if (fm_ctx->parent != NULL) { /* make clang-scan happy */
		fni_clear_subtree_mappings(fm_ctx->parent, fm_ctx->slot_idx);
	}
	return 0;
}
...
static int fmc_discard_data_at(const struct silofs_fmap_ctx *fm_ctx)
{
	...
	const int fl_mode = fm_ctx->f_ctx->fl_mode;
	const enum silofs_stype stype = vaddr_stype(&fm_ctx->vaddr);
	bool partial;

	if (fm_ctx->has_data) {
		partial = off_is_partial(fm_ctx->file_pos, end, stype);
		if (partial) {
			err = fmc_discard_partial(fm_ctx);
		} else if (fl_mode_zero_range(fl_mode)) {
			err = fmc_discard_by_set_unwritten(fm_ctx);
		} else {
			err = fmc_discard_entire(fm_ctx);
		}
	}
	return err;
}
...
static int fic_discard_by_tree_map(struct silofs_file_ctx *f_ctx)
{
	...
	while (fic_has_more_io(f_ctx)) {
		err = fic_seek_by_tree_map(f_ctx, &fm_ctx);
		if (err == -ENOENT) {
			break;
		}
		if (err) {
		...
		err = fmc_discard_data_at(&fm_ctx);
		if (err) {
			return err;
		}
		fic_advance_to_next(f_ctx);
...
static int fic_resolve_truncate_end(struct silofs_file_ctx *f_ctx)
{
	...
	f_ctx->end = off_max3(f_ctx->off, fic_head_leaves_end(f_ctx), tend);
	return 0;
}

static int fic_do_truncate(struct silofs_file_ctx *f_ctx)
{
	int err;
                               ...
                               struct silofs_fmap_ctx *fm_ctx)
{
	...
	err = fic_seek_data_by_head_leaves(f_ctx, fm_ctx);
	if (!err || (err != -ENOENT)) {
		return err;
	}
	err = fic_seek_by_tree_map(f_ctx, fm_ctx);
                                 ...
                                 struct silofs_fmap_ctx *fm_ctx)
{
	...
	err = fic_seek_hole_by_head_leaves(f_ctx, fm_ctx);
	if (!err || (err != -ENOENT)) {
		return err;
	}
	err = fic_seek_by_tree_map(f_ctx, fm_ctx);
...
static int fic_lseek_hole(struct silofs_file_ctx *f_ctx)
{
	...
	err = fic_lseek_hole_noleaf(f_ctx, &fm_ctx);
	if (err == 0) {
...
static int fic_do_lseek(struct silofs_file_ctx *f_ctx)
{
	int err;
	...
	if (fic_is_seek_data(f_ctx)) {
		err = fic_lseek_data(f_ctx);
	} else if (fic_is_seek_hole(f_ctx)) {
		err =  fic_lseek_hole(f_ctx);
	} else {
		err = fic_lseek_notsupp(f_ctx);
	}
	return err;
}
                                     ...
                                     struct silofs_finode_info *parent_fni)
{
	...
	fic_resolve_tree_leaf(f_ctx, parent_fni, &fm_ctx);
	if (fm_ctx.has_data) {
		return fmc_require_mutable(&fm_ctx);
	}
	err = fic_create_data_leaf(f_ctx, SILOFS_STYPE_DATABK, &fm_ctx.vaddr);
	if (err) {
		return err;
	}
	fni_bind_child(parent_fni, f_ctx->off, &fm_ctx.vaddr);
	return 0;
}

static int fic_do_reserve_tree_leaves(struct silofs_file_ctx *f_ctx,
                                      struct silofs_finode_info *parent_fni)
{
	int err;
	...
	while (fic_has_more_io(f_ctx) && !next_mapping) {
		err = fic_create_bind_tree_leaf(f_ctx, parent_fni);
		if (err) {
			return err;
		}
		fic_advance_to_next(f_ctx);
                                   ...
                                   struct silofs_finode_info *parent_fni)
{
	...
	fni_incref(parent_fni);
	err = fic_do_reserve_tree_leaves(f_ctx, parent_fni);
	fni_decref(parent_fni);
	return err;
}
...
static int fic_do_fallocate_op(struct silofs_file_ctx *f_ctx)
{
	int err;
...
static int fic_do_fallocate(struct silofs_file_ctx *f_ctx)
{
	int err;
                            ...
                            const struct silofs_fmap_ctx *fm_ctx)
{
	...
	if (fm_ctx->has_data) {
		ok = fic_emit_fiemap_ext(f_ctx, &fm_ctx->vaddr);
		if (!ok) {
			f_ctx->fm_stop = true;
		}
	}
...
static int fic_do_fiemap_by_tree_leaves(struct silofs_file_ctx *f_ctx,
                                        struct silofs_finode_info *parent_fni)
{
	struct silofs_fmap_ctx fm_ctx = { .f_ctx = NULL };

	while (fic_has_more_io(f_ctx)) {
		fic_resolve_tree_leaf(f_ctx, parent_fni, &fm_ctx);
		if (!fic_emit_fiemap(f_ctx, &fm_ctx)) {
			break;
		}
		fic_advance_to_next(f_ctx);
		if (fic_ismapping_boundaries(f_ctx)) {
                                     ...
                                     struct silofs_finode_info *parent_fni)
{
	...
	fni_incref(parent_fni);
	err = fic_do_fiemap_by_tree_leaves(f_ctx, parent_fni);
	fni_decref(parent_fni);
	return err;
}
...
static int fic_fiemap_by_tree_map(struct silofs_file_ctx *f_ctx)
{
	...
	while (fic_has_more_io(f_ctx)) {
		...
		err = fic_fiemap_by_tree_leaves(f_ctx, fm_ctx.parent);
		if (err) {
			return err;
		}
...
static int fic_do_fiemap(struct silofs_file_ctx *f_ctx)
{
	int err;
	...
	err = fic_check_fm_flags(f_ctx);
	if (err) {
		return err;
	}
	err = fic_fiemap_data(f_ctx);
	if (err) {
		return err;
	}
	return 0;
...
static int
fic_resolve_fmap_by_tree_recursive(struct silofs_file_ctx *f_ctx,
                                   struct silofs_finode_info *parent_fni,
                                   struct silofs_fmap_ctx *out_fm_ctx);

static int fic_resolve_fmap_by_head_leaves(struct silofs_file_ctx *f_ctx,
                struct silofs_fmap_ctx *out_fm_ctx)
{
	int err = 0;
	...
	if (fic_has_head1_leaves_io(f_ctx)) {
		...
		fmc_nomap(out_fm_ctx, f_ctx, NULL, f_ctx->off);
		err = -ENOENT;
	}
	return err;
}
...
static int
fic_resolve_fmap_recursive_at(struct silofs_file_ctx *f_ctx,
                              struct silofs_finode_info *parent_fni,
                              size_t slot, struct silofs_fmap_ctx *out_fm_ctx)
{
	int err;
	struct silofs_vaddr vaddr;
	struct silofs_finode_info *fni = NULL;
	...
	fni_resolve_child_by_slot(parent_fni, slot, &vaddr);
	fmc_nomap(out_fm_ctx, f_ctx, parent_fni, f_ctx->off);
	...
	err = fic_resolve_fmap_by_tree_recursive(f_ctx, fni, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
...
static int
fic_do_resolve_fmap_by_tree_rec(struct silofs_file_ctx *f_ctx,
                                struct silofs_finode_info *parent_fni,
                                struct silofs_fmap_ctx *out_fm_ctx)
{
	size_t slot;
	...
	if (!fni_isbottom(parent_fni)) {
		return fic_resolve_fmap_recursive_at(f_ctx, parent_fni,
		                                     slot, out_fm_ctx);
	}
	fic_resolve_child_at(f_ctx, parent_fni, f_ctx->off, slot, out_fm_ctx);
	return 0;
}
                                   ...
                                   struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	fni_incref(parent_fni);
	err = fic_do_resolve_fmap_by_tree_rec(f_ctx, parent_fni,
	                                      out_fm_ctx);
	fni_decref(parent_fni);
	return err;
}

static int fic_resolve_fmap_by_tree_map(struct silofs_file_ctx *f_ctx,
                                        struct silofs_fmap_ctx *out_fm_ctx)
{
	int err;
	struct silofs_finode_info *root_fni = NULL;
	...
	err = fic_resolve_fmap_by_tree_recursive(f_ctx, root_fni, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
...
static int fic_resolve_fmap_at(struct silofs_file_ctx *f_ctx,
                               struct silofs_fmap_ctx *out_fm_ctx)
{
	int err;

	err = fic_resolve_fmap_by_head_leaves(f_ctx, out_fm_ctx);
	if (err == -ENOENT) {
		err = fic_resolve_fmap_by_tree_map(f_ctx, out_fm_ctx);
	}
	return err;
}
...
static size_t fmc_copy_range_length(const struct silofs_fmap_ctx *fm_ctx_src,
                                    const struct silofs_fmap_ctx *fm_ctx_dst)
{
	const size_t len_src = fic_copy_length_of(fm_ctx_src->f_ctx);
	const size_t len_dst = fic_copy_length_of(fm_ctx_dst->f_ctx);
                                  ...
                                  struct silofs_fiovec *out_fiov)
{
	...
	err = fmc_stage_fileaf_at(fm_ctx, out_fli);
	if (err) {
		return err;
	}
                         ...
                         const struct silofs_fmap_ctx *fm_ctx_dst, size_t len)
{
	...
	err = fmc_prepare_unwritten_leaf(fm_ctx_src);
	if (err) {
		return err;
	}
	if (fic_has_kcopy_mode(fm_ctx_dst->f_ctx)) {
		...
		err = fmc_clear_unwritten_by(fm_ctx_dst);
		if (err) {
			return err;
		}
	} else {
		...
		err = fic_clear_unwritten_of(fm_ctx_dst->f_ctx, fli_dst);
		if (err) {
			return err;
		}
	}
                                  ...
                                  struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	err = fic_require_tree_leaf(f_ctx, parent_fni, out_fm_ctx);
	if (err) {
		return err;
	}
	return 0;
                            ...
                            struct silofs_fmap_ctx *out_fm_ctx)
{
	...
	if (off_is_head1(f_ctx->off)) {
		err = fic_require_head1_leaf(f_ctx, out_fm_ctx);
	} else if (off_is_head2(f_ctx->off)) {
		err = fic_require_head2_leaf(f_ctx, out_fm_ctx);
	} else {
		err = fic_require_tree_leaf2(f_ctx, out_fm_ctx);
	}
	return err;
}

static int
fmc_copy_range_at_leaf(const struct silofs_fmap_ctx *fm_ctx_src,
                       struct silofs_fmap_ctx *fm_ctx_dst, size_t *out_len)
{
	int err = 0;
	size_t len = 0;
	const struct silofs_file_ctx *f_ctx_dst = fm_ctx_dst->f_ctx;

	len = fmc_copy_range_length(fm_ctx_src, fm_ctx_dst);
	if (!fm_ctx_src->has_data) {
		if (fm_ctx_dst->has_data) {
			err = fmc_require_mutable(fm_ctx_dst);
			if (err) {
				return err;
			}
			err = fmc_discard_data_at(fm_ctx_dst);
			if (err) {
				return err;
			}
		}
		...
		if (!fm_ctx_dst->has_data) {
			err = fic_require_leaf(f_ctx_dst, fm_ctx_dst);
			if (err) {
				return err;
			}
		} else {
		...
		err = fmc_copy_leaf(fm_ctx_src, fm_ctx_dst, len);
		if (err) {
			return err;
		}
	}
                               ...
                               struct silofs_file_ctx *f_ctx_dst)
{
	int err;
	size_t len;
	struct silofs_fmap_ctx fm_ctx_src;
	struct silofs_fmap_ctx fm_ctx_dst;

	while (fic_has_more_io(f_ctx_src) && fic_has_more_io(f_ctx_dst)) {
		err = fic_resolve_fmap_at(f_ctx_src, &fm_ctx_src);
		if (err && (err != -ENOENT)) {
			return err;
		}
		err = fic_resolve_fmap_at(f_ctx_dst, &fm_ctx_dst);
		if (err && (err != -ENOENT)) {
			return err;
		}
		err = fmc_copy_range_at_leaf(&fm_ctx_src, &fm_ctx_dst, &len);
		if (err) {
			return err;
		}
...
static int
fic_do_copy_range(struct silofs_file_ctx *f_ctx_src,
                              ...
                              int flags, size_t *out_ncp)
{
	...
	ii_incref(ii_in);
	ii_incref(ii_out);
	err = fic_do_copy_range(&f_ctx_in, &f_ctx_out, out_ncp);
	ii_decref(ii_out);
	ii_decref(ii_in);
	return err;
}
