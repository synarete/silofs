...
#define SILOFS_IO_NBK_MAX \
	(SILOFS_FILE_HEAD2_NLEAVES + (SILOFS_IO_SIZE_MAX / SILOFS_BK_SIZE))
...
struct silofs_fuseq_wr_iter {
	struct silofs_fiovec fiov[SILOFS_IO_NBK_MAX];
                                    ...
                                    size_t len)
{
	...
	return silofs_pipe_append_from_buf(&fqw->pipe, &hdr, sizeof(hdr));
}
                                      ...
                                      const struct silofs_fiovec *fiov)
{
	...
	return silofs_pipe_splice_from_fd(&fqw->pipe, fiov->fv_fd, &off, len);
}
                                       ...
                                       const struct silofs_fiovec *fiov)
{
	...
	return silofs_pipe_vmsplice_from_iov(&fqw->pipe, &iov, 1);
}
...
static int fuseq_send_pipe(struct silofs_fuseq_worker *fqw)
{
	...
	err = silofs_pipe_flush_to_fd(&fqw->pipe, fqw->fq->fq_fuse_fd);
	return err;
}
                   ...
                   const struct silofs_fuseq_in *in)
{
	...
	apex->fa_sbi->s_mntime = silofs_time_now();
                              ...
                              const struct silofs_fiovec *fiov)
{
	...
	return silofs_pipe_splice_to_fd(&fqw->pipe, fiov->fv_fd,
	                                &off, fiov->fv_len);
}
                               ...
                               const struct silofs_fiovec *fiov)
{
	...
	return silofs_pipe_vmsplice_to_iov(&fqw->pipe, &iov, 1);
}
...
{
	fqw->oper.ucred.uid = (uid_t)(hdr->uid);
	fqw->oper.ucred.gid = (gid_t)(hdr->gid);
	fqw->oper.ucred.pid = (pid_t)(hdr->pid);
	fqw->oper.ucred.umask = 0;
	fqw->oper.unique = hdr->unique;
	fqw->oper.opcode = (int)hdr->opcode;
}
...
static int fuseq_setup_curr_xtime(struct silofs_fuseq_worker *fqw)
{
	...
	return silofs_ts_gettime(&fqw->oper.xtime, is_realtime);
}
...
static void fuseq_interrupt_op(struct silofs_fuseq_worker *fqw, uint64_t unq)
{
	...
	while (itr != &lsq->ls) {
		...
		if (fqw_other->oper.unique == unq) {
			fqw_other->oper.interrupt++;
			break;
		}
		itr = itr->next;
	}
...
{
	const struct silofs_pipe *pipe = &fqw->pipe;
...
{
	const int fuse_fd = fqw->fq->fq_fuse_fd;
	struct silofs_pipe *pipe = &fqw->pipe;
...
static int fuseq_splice_in(struct silofs_fuseq_worker *fqw)
{
	...
	err = silofs_pipe_copy_to_buf(pipe, tail, rem);
	if (err) {
...
static int fuseq_prep_request(struct silofs_fuseq_worker *fqw)
{
	...
	return silofs_pipe_dispose(&fqw->pipe, &fqw->nfd);
}
...
static int fuseq_init_conn_info(struct silofs_fuseq *fq)
{
	...
	fq->fq_coni.congestion_threshold = 12; /* XXX crap2 */
	fq->fq_coni.time_gran = 1; /* XXX Is it ? */
	return 0;
}

static int fuseq_init_pipe(struct silofs_fuseq_worker *fqw, size_t pipe_size)
{
	int err;
	struct silofs_pipe *pipe = &fqw->pipe;
...
static void fuseq_fini_pipe(struct silofs_fuseq_worker *fqw)
{
	silofs_pipe_fini(&fqw->pipe);
}

static int fuseq_init_nilfd(struct silofs_fuseq_worker *fqw)
{
	return silofs_nilfd_init(&fqw->nfd);
}

static void fuseq_fini_nilfd(struct silofs_fuseq_worker *fqw)
{
	silofs_nilfd_fini(&fqw->nfd);
}
...
static int fuseq_init_worker(struct silofs_fuseq_worker *fqw,
                             struct silofs_fuseq *fq, int idx)
{
	int err;
	...
	err = fuseq_init_pipe(fqw, pipe_size_want);
	if (err) {
		goto out;
	}
	err = fuseq_init_nilfd(fqw);
	if (err) {
		goto out;
	}
out:
	if (err) {
		fuseq_fini_nilfd(fqw);
		fuseq_fini_pipe(fqw);
		fuseq_fini_rwi(fqw);
		fuseq_fini_bufs(fqw);
	}
...
static int fuseq_init_workers(struct silofs_fuseq *fq)
{
	...
	mem_size = (size_t)nlimit * sizeof(*fws->fws_workers);
                       ...
                       struct silofs_fs_apex *apex, const char *path)
{
	...
	struct silofs_sb_info *sbi = apex->fa_sbi;
...
static int fuseq_exec_thread(struct silofs_fuseq_worker *fqw)
{
	...
	snprintf(name, sizeof(name) - 1, "silofs-%d", fqw->worker_index + 1);
