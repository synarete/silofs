...
{
	iova->ino = ino;
	silofs_ovaddr_setup(&iova->ova, oaddr, vaddr);
}
...
static int spc_stage_spnode(struct silofs_spalloc_ctx *spa_ctx,
                            loff_t voff, enum silofs_stage_flags stgf)
{
	return silofs_stage_spnode(spa_ctx->sbi, voff, stgf, &spa_ctx->sni);
}
...
static int spc_stage_spleaf(struct silofs_spalloc_ctx *spa_ctx,
                            loff_t voff, enum silofs_stage_flags stgf)
{
	int err;

	silofs_sni_incref(spa_ctx->sni);
	err = silofs_stage_spleaf(spa_ctx->sbi, voff, stgf, &spa_ctx->sli);
...
static int spc_resolve_oaddr(struct silofs_spalloc_ctx *spa_ctx)
{
	...
	return silofs_resolve_oaddr(spa_ctx->sbi, vaddr, &spa_ctx->ova.oaddr);
}
...
static int spc_do_find_free_at_node(struct silofs_spalloc_ctx *spa_ctx)
{
	int err;
...
static int spc_find_free_at_node(struct silofs_spalloc_ctx *spa_ctx)
{
	...
	silofs_sni_incref(spa_ctx->sni);
	err = spc_do_find_free_at_node(spa_ctx);
	silofs_sni_decref(spa_ctx->sni);
	return err;
}
                        ...
                        struct silofs_ovaddr *out_ova)
{
	...
	silofs_ovaddr_assign(out_ova, &spa_ctx.ova);
	return 0;
}

static int
spc_spawn_mutable_vnode_at(const struct silofs_spalloc_ctx *spa_ctx,
                           struct silofs_vnode_info **out_vi)
{
	return silofs_spawn_bind_vnode_at(spa_ctx->sbi, &spa_ctx->ova,
                             ...
                             struct silofs_vnode_info **out_vi)
{
	...
	err = spc_spawn_mutable_vnode_at(&spa_ctx, out_vi);
	if (err) {
                             ...
                             struct silofs_inode_info **out_ii)
{
	...
	err = spc_spawn_mutable_vnode_at(&spa_ctx, &vi);
	if (err) {
		return err;
	}
