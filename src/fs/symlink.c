                               ...
                               struct silofs_symval_info **out_syi)
{
	...
	err = silofs_stage_vnode(sl_ctx->sbi, vaddr, sl_ctx->stg_flags, &vi);
	if (err) {
		return err;
	}
                            ...
                            struct silofs_symval_info **out_syi)
{
	...
	err = slc_do_stage_symval(sl_ctx, vaddr, out_syi);
...
static int slc_do_symlink(struct silofs_symlnk_ctx *sl_ctx)
{
	int err;
