...
void ut_format_fs(struct ut_env *ute)
{
	...
	ut_expect_ok(err);
	err = silofs_get_fs_xref(ute->env, &ute->bootrec_xref[0]);
	ut_expect_ok(err);
}
...
void ut_open_fs(struct ut_env *ute)
{
	...
	err = silofs_set_fs_xref(ute->env, &ute->bootrec_xref[0]);
	ut_expect_ok(err);
...
void ut_open_fs2(struct ut_env *ute)
{
	...
	err = silofs_set_fs_xref(ute->env, &ute->bootrec_xref[1]);
	ut_expect_ok(err);
...
void ut_fork_fs(struct ut_env *ute)
{
	...
	err = silofs_get_fs_fork_xref(ute->env, &ute->bootrec_xref[1]);
	ut_expect_ok(err);
}
