...
static void ut_file_fallocate_rdonly(struct ut_env *ute)
{
	const struct ut_range ranges[] = {
		...
		UT_MKRANGE1(1, UT_1M),
		UT_MKRANGE1(0, UT_1M),
		UT_MKRANGE1(UT_1M - 1, UT_1M + 2),
		UT_MKRANGE1(UT_1G, UT_1M),
		UT_MKRANGE1(UT_1T - 2, UT_1M + 3),
	};
...
static void
ut_file_fallocate_punch_hole_sparse_(struct ut_env *ute, off_t off_base,
                                     off_t step, size_t cnt)
{
	...
	for (size_t i = 0; i < cnt; ++i) {
		off = off_base + ((off_t)i * step);
		ut_write_read1(ute, ino, off);
		off = off + (3 * bk_size);
		ut_write_read1(ute, ino, off);
		off = off_base + ((off_t)(i + 1) * step);
