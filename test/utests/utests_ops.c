...
static void ut_cache_nodes(const struct ut_env *ute, size_t *out_lnodes,
                           size_t *out_pnodes)
{
	*out_lnodes = ute->env->lcache.nc.hmapq.hmq_lru.sz;
	*out_pnodes = ute->env->pcache.nc.hmapq.hmq_lru.sz;
}
...
void ut_remove_fs(struct ut_env *ute)
{
	...
	err = silofs_remove_fs(ute->env, &ute->fsrefs.main);
	ut_expect_ok(err);
}
...
void ut_remove_fs2(struct ut_env *ute)
{
	...
	err = silofs_remove_fs(ute->env, &ute->fsrefs.fork);
	ut_expect_ok(err);
}
