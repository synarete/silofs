...
{
	ut_save_bsec_at(ute, true, ute->fs_env->fs_args.warm_name, bsec);
}

void ut_load_bsec_ok(struct ut_env *ute, struct silofs_bootsec *bsec)
{
	ut_load_bsec_at(ute, true, ute->fs_env->fs_args.warm_name, bsec);
}
                              ...
                              const char *name, bool main)
{
	...
	if (main) {
		repodir = ute->fs_env->fs_args.warm_repodir;
		if (name == NULL) {
			name = ute->fs_env->fs_args.warm_name;
		}
	} else {
...
{
	const char *fsname = ute->fs_env->fs_args.warm_name;
