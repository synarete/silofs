...
void ut_format_fs_ok(struct ut_env *ute)
{
	...
	err = silofs_format_fs(ute->fs_ctx, &ute->treeid[0]);
	ut_expect_ok(err);
}
...
void ut_open_fs_ok(struct ut_env *ute)
{
	...
	err = silofs_boot_fs(ute->fs_ctx, &ute->treeid[0]);
	ut_expect_ok(err);
...
void ut_open_fs2_ok(struct ut_env *ute)
{
	...
	err = silofs_boot_fs(ute->fs_ctx, &ute->treeid[1]);
	ut_expect_ok(err);
...
void ut_unref_fs2_ok(struct ut_env *ute)
{
	...
	err = silofs_unref_fs(ute->fs_ctx, &ute->treeid[1]);
	ut_expect_ok(err);
}
...
void ut_fork_fs_ok(struct ut_env *ute)
{
	...
	err = silofs_fork_fs(ute->fs_ctx, &ute->treeid[0], &ute->treeid[1]);
	ut_expect_ok(err);
}
